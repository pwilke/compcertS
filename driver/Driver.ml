...
(*                                                                     *)
(* *********************************************************************)
(*peq stdout_id i/peq stdout_id ii*)
open Printf
open Camlcoq
open Clflags

(* Display information about extra blocks *)
let info_extra = ref false
...
(* let list_glob p = *)
(*   let genv = Globalenvs.Genv.globalenv p in *)
(*   let n = Globalenvs.Genv.genv_next genv in *)
(*   List.fold_left (fun acc elt -> *)
(* 		  let b = Camlcoq.P.of_int elt in *)
(* 		  match Globalenvs.Genv.find_funct_ptr genv b with *)
(* 		    Some (AST.Internal f) -> *)
(* 		    begin *)
(* 		      match Globalenvs.Genv.invert_symbol genv b with *)
(* 			Some sym -> *)
(* 			Hashtbl.replace func_names (f.Asm.fn_id) (extern_atom sym); *)
(* 			(sym,f) :: acc *)
(* 		      | _ -> acc *)
(* 		    end *)
(* 		  | _ -> acc *)
(* 		 ) [] (list_ints (Camlcoq.P.to_int n)) *)

(* let show_functions_extra lg = *)
(*   List.iter (fun (i,x) -> *)
(* 	     Hashtbl.replace func_extra_size (x.Asm.fn_id) *)
(* 			     ((Camlcoq.Nat.to_int (x.Asm.fn_nbextra)), *)
(* 			     (Camlcoq.Z.to_int (x.Asm.fn_stacksize)))) *)
(* 	    lg *)


(* 
 * ns : stack needed at C level (provision)
 * sl : stack freed by SimplLocals
 *)
(* let do_dump filename asm ((sg,ns),sl) : unit =  *)
(*   let lg = list_glob asm in *)
(*   begin *)
(*     show_functions_extra lg; *)
(*     let oc = open_out "dump.tex" in *)
(*     fprintf oc "\\documentclass{article} *)
(*   		\\usepackage[a0paper]{geometry} *)
(*   		\\usepackage{tikz} *)
(*   		\\usetikzlibrary{positioning,calc} *)
(*   		\\begin{document} *)
(*   		"; *)
(*     let oc' = open_out_gen [Open_append] 0o755 "bench" in *)
(*     fprintf oc' "file\tfunc\tC variables\tprovision\treuse\tASM stacksize\tcalcul\n"; *)
(*     Hashtbl.iter *)
(*       (fun k name -> *)
(*        let cusage = try Hashtbl.find func_C_usage k with _ -> [-1] in *)
(*        let (extra,size) = try Hashtbl.find func_extra_size k with _ -> (-1,-1) in *)
(*        let ar = try Hashtbl.find func_stacking k with _ -> "no drawaing available; check Stacking.ml" in *)
(*        fprintf oc "\\section{Function %s}\n %d variables %s -- %d extra bytes -- total stack size: %d \\\\ \n%s\n" *)
(*   	       (Str.global_replace (Str.regexp "_") "\\_" name) *)
(*   	       (List.length cusage) *)
(*   	       ((List.fold_left *)
(*   		   (fun acc elt -> acc ^ " " ^ (string_of_int elt) ^",") "[" cusage)^"]") *)
(*   	       (ns k) size ar; *)
(*        let szvars = List.fold_left (fun acc elt -> acc + elt) 0 cusage in *)
(*        fprintf oc' "%s\t%s\t%d\t%d\t%d\t%d\t%d\n" *)
(*   	       filename name szvars (ns k) (sl k) *)
(*   	       size (size - 8*(ns k) - szvars) *)
(*       ) func_names; *)
(*         fprintf oc "\\end{document}\n"; *)
(*     close_out oc; *)
(*     close_out oc' *)
(*   end *)
    
(* let do_dump filename asm (ns) : unit =  *)
(*   let lg = list_glob asm in *)
(*   begin *)
(*     show_functions_extra lg; *)
(*     let oc = open_out "dump.tex" in *)
...
(*   		\\begin{document} *)
(*   		"; *)
(*     fprintf oc' "file\tfunc\tC variables\tC variables + extra\tASM stacksize\n"; *)
(*     Hashtbl.iter *)
(*       (fun k name -> *)
(*        let cusage = try Hashtbl.find func_C_usage k with _ -> [-1] in *)
(*        let (extra,size) = try Hashtbl.find func_extra_size k with _ -> (-1,-1) in *)
...
(*   	       (ns k) size ar; *)
(*        let szvars = List.fold_left (fun acc elt -> acc + elt) 0 cusage in *)
(*        fprintf oc' "%s\t%s\t%d\t%d\t%d\n" *)
(*   	       filename name szvars (ns k) *)
(*   	       size *)
(*       ) func_names; *)
(*     fprintf oc "\\end{document}\n"; *)
(*     close_out oc; *)
...
(* let oracles p = match Compiler.do_oracles p with *)
(*     Some ((sg,ns),sl) -> *)
(*     ((fun i -> sg i |> Camlcoq.Z.to_int ), *)
(*      (fun i -> ns i |> Camlcoq.Nat.to_int)), *)
(*      (fun i -> sl i |> Camlcoq.Nat.to_int) *)
(*   | None -> ((fun i -> -1), *)
(* 	     (fun i -> -1)), *)
(* 	     (fun i -> -1) *)
...
let compile_c_ast sourcename csyntax ofile =
  ...
  (* Convert to Asm *)
  let asm = match Compiler.transf_c_program csyntax with
    | Errors.OK asm -> asm
       (* do_dump sourcename asm (oracles csyntax);  *)
...
General options:
  ...
  -info-extra    Display information about extra blocks used
Interpreter mode:
...
let cmdline_actions =
  ...
  "-time_trace$", String(fun t -> Interp.time_trace := int_of_string t);
  "-quiet$", Self (fun _ -> Interp.trace := 0);
  "-trace$", Self (fun _ -> Interp.trace := 2);
  "-mem_trace$", Self (fun _ -> Interp.mem_debug := true);
  "-dummy-blocks$", String(fun t -> Interp.dummy_blocks := int_of_string t);
  "-dummy-size$", String(fun t -> Interp.dummy_size := int_of_string t);
  "-info-extra$", Set info_extra;
