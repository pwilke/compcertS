...
Proof.
  ...
  exploit A; eauto. intro EQ; rewrite FB in EQ; inv EQ.
...
Inductive volatile_load (F V: Type) (ge: Genv.t F V):
  ...
  | volatile_load_vol: forall chunk m b ofs id ev v,
      ...
      Genv.find_symbol ge id = Some b ->
      CCMemory.Mem.range_perm m b (Int.unsigned ofs) (Int.unsigned ofs + size_chunk chunk) Cur Readable ->
...
Inductive volatile_store (F V: Type) (ge: Genv.t F V):
  ...
  | volatile_store_vol: forall chunk m b ofs id ev v,
      block_is_volatile ge b = true ->
      Genv.find_symbol ge id = Some b ->
      CCMemory.Mem.range_perm m b (Int.unsigned ofs) (Int.unsigned ofs + size_chunk chunk) Cur Writable ->
...
Proof.
  ...
  red; intros. eapply Mem.perm_extends; eauto.
...
Proof.
  intros. inv H0.
  inv H1. exploit (proj1 H); eauto. intros EQ; rewrite FB in EQ; inv EQ.
  rewrite Int.add_zero. exists (Val.load_result chunk v0); split.
  constructor; auto.
  red; intros. exploit Mem.perm_inject; eauto. rewrite Z.add_0_r. auto.
...
(* mem extends *)
  inv H. inv H1. inv LDL. inv LD. 
  exploit volatile_load_extends; eauto. intros [v' [A B]].
  exists v'; exists m1'; intuition. constructor; auto.
(* mem injects *)
  inv H0. inv H2. inv VLI. inversion VI; subst. 
...
(* mem extends*)
  inv H. inv H1. inv LDL. inv LDL0. inv LD.
  exploit volatile_store_extends; eauto. intros [m2' [A [B C]]]. 
  exists Vundef; exists m2'; intuition. constructor; auto. 
(* mem inject *)
  inv H0. inv H2. inv VLI. inv VLI0. inversion VI; subst.
...
(* mem inject *)
  ...
  split. eapply Mem.free_unchanged_on; eauto. unfold loc_out_of_reach. 
    intros. red; intros. eelim H6; eauto.
    apply Mem.perm_cur_max. apply Mem.perm_implies with Freeable; auto with mem. 
    apply H0. omega.
  split. auto. 
  red; intros. congruence.
...
- (* extensions *)
  intros. inv H. 
  inv H1. inv LDL. inv LDL0. inv LD. inv LD0.
  ...
  tauto.
- (* injections *)
  intros. inv H0. inv H2. inv VLI. inv VLI0. inv VI0. inv VI.
  destruct (zeq sz 0).
+ (* special case sz = 0 *)
  assert (bytes = nil). 
...
(* readonly *)
  inv H. apply Mem.unchanged_on_refl.
(* mem extends *)
  inv H. inv H1. inv LDL.
  exists v2; exists m1'; intuition.
  econstructor; eauto.
  eapply eventval_match_lessdef; eauto.
(* mem inject *)
  inv H0. inv H2. inv VLI.
...
Proof.
  induction tyl; simpl; intros. 
  auto.
  destruct a; inv H; auto. inv VLI; auto. constructor; auto. apply val_longofwords_inject; auto. 
Qed.
