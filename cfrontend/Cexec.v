...
Proof.
  intros.
  des v; des t; try destr_in H; inv H; try constructor.
...
Proof.
  ...
  rewrite (Genv.find_invert_symbol _ _ FS). auto.
Qed.
...
Proof with try discriminate.
  induction vl; destruct tl; simpl; intros;
  repeat destr_in H; inv H;
  try constructor.
  apply eventval_of_val_sound; auto.  eauto.
Qed.
...
Proof.
  induction 1; simpl. auto.
  apply eventval_of_val_complete in EVM. rewrite EVM.
  rewrite IHeventval_list_match. auto.
Qed.
...
Proof.
  intros.
  des ev; des t; repeat destr_in H; inv H; try constructor; auto.
Qed.
...
Proof.
  unfold in_bound, Mem.in_bound_m. intros.
  destr. destr. split; intros; auto. red. simpl.
  rewrite Mem.zle_zlt in Heqb0. auto.
  split; intros; try congruence.
  red in H. simpl in H. rewrite <- Mem.zle_zlt in H. congruence.
Qed.
...
Proof.
  intros until v. unfold do_volatile_load. destr_cond_match; try discriminate. mydestr. 
  destruct p as [ev w'']. mydestr. 
  split.
  econstructor; eauto.
  rewrite in_bound_true in Heqb1; auto.
  apply Genv.invert_find_symbol; auto.
...
Proof.
  ...
  rewrite MN. rewrite VOL.
  rewrite (Genv.find_invert_symbol _ _ FS). inv H0. inv H5. inv H3. rewrite H6.
  rewrite (val_of_eventval_complete _ _ _ EVM).
  rewrite (proj2 (in_bound_true _ _ _)); auto.
  rewrite MN, VOL, LOAD; auto. inv H0; auto.
Qed.
...
Proof.
  intros until m'. unfold do_volatile_store.
  destr_cond_match; try discriminate.
  mydestr. 
  split. econstructor; eauto. rewrite in_bound_true in Heqb1; auto.
  apply Genv.invert_find_symbol; auto.
...
Proof.
  ...
  destr. inv MN.
  rewrite VOL.
  rewrite (Genv.find_invert_symbol _ _ FS). 
  inv H0.
  rewrite (proj2 (in_bound_true _ _ _)); auto.
  inv H5. inv H3.
  apply eventval_of_val_complete in EVM. rewrite EVM. rewrite H6. auto.
  rewrite MN, VOL, STORE. inv H0. auto.
Qed.
...
Proof.
  intros; destruct b. unfold NormaliseSpec.in_bound. simpl in *.
  destruct (zle z o); destruct (zlt o z0); auto;
  right; omega.
Defined.
...
Proof.
  ...
  split. econstructor 4; eauto. constructor.
  destruct (Mem.mem_norm m v) eqn:?; try discriminate.
  destruct (Mem.mem_norm m addr) eqn:?; try discriminate.
  mydestr.
  destruct a as [P [Q R]]. 
  split.
  eapply assign_loc_copy with (ofs':=i); eauto. generalize (sizeof_pos ty). Psatz.lia.
  constructor.
Qed.
...
Proof.
  ...
  - (* by copy *)
    rewrite H2, H8, H9.
    destr_cond_match; try Psatz.lia.
...
(* Definition ptr_appears_dec: *)
(*   forall b c, *)
(*     {NormaliseSpec.ptr_appears b c} + *)
(*     {~ NormaliseSpec.ptr_appears b c}. *)
(* Proof. *)
(*   induction c; simpl; intros; auto. *)
(*   - destruct s; simpl; auto. *)
(*     decide equality. *)
(*   - destruct IHc1; destruct IHc2; auto. *)
(*     right; intros [A|A]; congruence. *)
(* Qed. *)
...
Proof with try congruence.
    ...
    volatile_load_sem chunk ge vargs m t vres m' /\ possible_trace w t w').
  {
    ...
    repeat destr_cond_match; try discriminate. subst.
    intro A; inv A.
                    ...
                    volatile_store_sem chunk ge vargs m t vres m' /\ possible_trace w t w').
  {
    ...
    repeat destr_cond_match; try discriminate; subst. intro A; inv A. 
...
- unfold do_ef_malloc.
  Require Import Tactics. 
  repeat destr. intro A; inv A.
  split. econstructor. eauto. eauto. eauto. constructor.
- unfold do_ef_free.
  repeat destr.
  intro A; inv A.
  split. econstructor. eauto. red. auto. eauto. eauto. 
  omega. auto. constructor.
- (* EF_memcpy *)
  unfold do_ef_memcpy.
  repeat destr.
  + intro A; inv A. split.
    econstructor 2; eauto.
    apply possible_trace_nil.
  + intro A; inv A.
    split.
    destruct m0 as (A & B & C & D & E & F).
    assert (sz > 0) by Psatz.lia.
    repeat match goal with
             A: sz > 0 -> _ |- _ => try specialize (A H)
           end.
    eapply (extcall_memcpy_sem_intro). 7: eauto. 7: eauto.
    auto.
    auto.
    auto.
    auto.
...
- unfold do_ef_annot. destr. intro A; inv A.
  split. constructor.
  eapply list_eventval_of_val_sound; eauto.
  repeat econstructor.
- unfold do_ef_annot_val. repeat destr. intro A; inv A.
  split. constructor.
...
Proof.
         ...
         ).
  {
    ...
    exploit do_volatile_load_complete; eauto. intro VL; rewrite VL; split; auto. 
  }
...
- (* EF_vstore *)
  ...
  unfold do_ef_volatile_store_global. rewrite P. rewrite VSTORE; auto.
- inv H. unfold do_ef_malloc. rewrite MN. rewrite ALLOC, STORE.
  inv H0. auto.  
- inv H; inv H0. simpl.
  rewrite MN, LOAD, MNsz, is_dynamic.
  destr; try omega.
  rewrite FREE. auto.  
- (* EF_memcpy *)
  inv H; inv H0; unfold do_ef_memcpy. 
  + destr. omega. 
    rewrite MNv, MNv'. 
    rewrite LB, SB.
...
- inv H; inv H0. inv H6.
  unfold do_ef_annot.
  rewrite (list_eventval_of_val_complete _ _ _ H1). inv H4. auto.
- inv H; inv H0. inv H4. inv H6. simpl.
  erewrite eventval_of_val_complete. eauto. auto.
- (* EF_inline_asm *)
...
Proof with (unfold ret; auto with coqlib).
  ...
  - (* Statement step *)
    inv H; simpl; auto;
    repeat match goal with
               (* H : Mem.mem_norm ?m ?v ?t = _ |- context [ Mem.mem_norm ?m ?v ?t ] *)
               H : ?a = _ |- context [ ?a ]
               => rewrite H; simpl
           end;
