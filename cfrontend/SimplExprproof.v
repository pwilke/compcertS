...
Lemma functions_translated:
  ...
  exists tf,
  Genv.find_funct m tge v = Some tf /\ tr_fundef f tf.
Proof.
...
Remark deref_loc_translated:
  forall ty m addr t v,
    Csem.deref_loc ge ty m addr t v ->
    match chunk_for_volatile_type ty with
      | None => t = E0 /\ Clight.deref_loc ty m addr v
      | Some chunk => volatile_load tge chunk m addr t v
    end.
Proof.
  ...
  - (* By_value, volatile *)
    ...
    inv H2.
    + eapply volatile_load_preserved with (ge1 := ge); eauto.
...
Remark assign_loc_translated:
  forall ty m addr v t m',
  Csem.assign_loc ge ty m addr v t m' ->
  match chunk_for_volatile_type ty with
  | None => t = E0 /\ Clight.assign_loc ty m addr v m'
  | Some chunk => volatile_store tge chunk m addr v t m'
  end.
Proof.
  intros. unfold chunk_for_volatile_type. inv H; auto.
  - (* By_value, not volatile *)
    rewrite H1. split; auto. eapply assign_loc_value; eauto.
  - (* By_value, volatile *)
    rewrite H0; rewrite H1.
    assert (exists b o, Mem.mem_norm m addr = Vptr b o).
    inv H2; eexists;  eauto.
    destruct H as [b [o H]].
    eapply volatile_store_preserved with (ge1 := ge); eauto.
    exact symbols_preserved. exact block_is_volatile_preserved.
  - (* By copy *)
    rewrite H1. destruct (type_is_volatile ty); split; auto; 
                eapply assign_loc_copy with (ofs':=ofs'); eauto.
  - rewrite H1.  destruct (type_is_volatile ty); split; auto; 
                eapply assign_loc_copy_0; eauto.
Qed.
...
Lemma tr_simple:
  forall e m,
    ...
    /\
    (forall l addr,
       eval_simple_lvalue ge e m l addr ->
       forall le sl a tmps,
         tr_expr le For_val l sl a tmps ->
         sl = nil /\ Csyntax.typeof l = typeof a /\ eval_lvalue tge e le m a addr).
Proof.
  Opaque makeif.
  intros e m.
  ...
  - (* addrof *)
    exploit H0; eauto. intros [A [B C]].
    subst sl1; simpl.
    assert (eval_expr tge e le m (Eaddrof a1 ty) ptr). econstructor; eauto.
    destruct dst; auto. simpl; econstructor; eauto. 
  - (* unop *)
    exploit H0; eauto. intros [A [B C]].
    subst sl1; simpl.
    assert (eval_expr tge e le m (ClightSyntax.Eunop op a1 ty) 
                      (sem_unary_operation_expr op v1 (Csyntax.typeof r1))). econstructor; eauto. congruence. 
    destruct dst; auto. simpl; econstructor; eauto. 
  - (* binop *)
    exploit H0; eauto. intros [A [B C]].
    exploit H2; eauto. intros [D [E F]].
    subst sl1 sl2; simpl.
    assert (eval_expr tge e le m (ClightSyntax.Ebinop op a1 a2 ty) v). econstructor; eauto. congruence.
    destruct dst; auto. simpl; econstructor; eauto.
  - (* cast *)
    exploit H0; eauto. intros [A [B C]].
    subst sl1; simpl.
...
Lemma step_makeif:
  ...
  star (fun ge => step1 ge needed_stackspace) tge (State f (makeif a s1 s2) k e le m)
...
Lemma step_make_set:
  forall id a ty m addr t v e le f k,
  Csem.deref_loc ge ty m addr t v ->
  eval_lvalue tge e le m a addr ->
  typeof a = ty ->
  step1 tge needed_stackspace (State f (make_set id a) k e le m)
...
- (* volatile case *)
  ...
  red; intros. simpl. 
  exploit Mem.norm_type; eauto. congruence.
  simpl. instantiate (1:=Tint); simpl;auto.
  intro WT; generalize (ExprEval.expr_type _ _ WT cm em); eauto.
  ExprEval.seval.
  rewrite Int.sign_ext_above with (n:=32%Z); auto; NormaliseSpec.unfsize; Psatz.lia.
  red; intros.
  exploit Mem.norm_type; eauto. congruence.
  simpl. instantiate (1:=Tint); simpl;auto.
  intro WT; generalize (ExprEval.expr_type _ _ WT cm em); eauto.
...
Lemma step_make_assign:
  forall a1 a2 ty m addr t v m' v2 e le f k,
  Csem.assign_loc ge ty m addr v t m' ->
  eval_lvalue tge e le m a1 addr ->
  eval_expr tge e le m a2 v2 ->
  sem_cast_expr v2 (typeof a2) ty = Some v ->
  typeof a1 = ty ->
  step1 tge needed_stackspace (State f (make_assign a1 a2) k e le m)
...
- (* volatile case *)
  intros. change le with (set_opttemp None (Eval Vundef) le) at 2. econstructor.
  econstructor. constructor. eauto.
  simpl. unfold sem_cast_expr. simpl. eauto.
  ...
  erewrite Mem.same_eval_eqm; eauto;
  red; intros;
  exploit (fun m e v => Mem.norm_type m e v Tint); eauto; try congruence;
  simpl; auto;
  intro WT; generalize (ExprEval.expr_type _ _ WT cm em); eauto;
  ExprEval.seval;
  rewrite Int.sign_ext_above with (n:=32%Z); auto; NormaliseSpec.unfsize; Psatz.lia.
...
Lemma push_seq:
  ...
  star (fun ge => step1 ge needed_stackspace) tge (State f (makeseq sl) k e le m)
...
Lemma step_tr_rvalof:
  forall ty m addr t v e le a sl a' tmp f k,
  Csem.deref_loc ge ty m addr t v ->
  eval_lvalue tge e le m a addr ->
  tr_rvalof ty a sl a' tmp ->
  typeof a = ty ->
  exists le',
    star (fun ge => step1 ge needed_stackspace) tge (State f Sskip (Kseqlist sl k) e le m)
                 t (State f Sskip k e le' m)
...
Proof.
  ...
  eapply tr_find_label_top with (e := empty_env) (le := PTree.empty expr_sym) (m := Mem.empty).
  eauto. apply H.
Qed.
...
Ltac clear_tr_top :=
  try match goal with
    H9 : tr_top _ _ _ _ _ _ _ _ _ |- _ => clear H9
  end.
...
Lemma estep_simulation:
  ...
  exists S2',
     (plus (fun ge => step1 ge needed_stackspace) tge S1' t S2' \/
       (star (fun ge => step1 ge needed_stackspace) tge S1' t S2' /\ measure S2 < measure S1)%nat)
  /\ match_states S2 S2'.
Proof.
...
- (* expr *)
  ...
  + (* for set *)
    inv H10.
- (* rval volatile *)
  exploit tr_top_leftcontext; eauto. clear_tr_top.
  intros [dst' [sl1 [sl2 [a' [tmp' [P [Q [R S]]]]]]]].
...
- (* seqand true *)
  ...
  eapply star_trans. apply step_makeif with (b := Int.one) (v1 := v); auto. 
  setoid_rewrite <- H1.
  congruence.
...
- (* seqand false *)
  ...
  econstructor; split.
  left. eapply plus_left. constructor.
  eapply star_trans. apply step_makeif with (b := Int.zero) (v1 := v); auto. 
  setoid_rewrite <- H1. congruence.
  ...
  econstructor; split.
  left. eapply plus_left. constructor.
  apply step_makeif with (b := Int.zero) (v1 := v); auto. 
  setoid_rewrite <- H1. congruence.
  reflexivity. 
  eapply match_exprstates; eauto.
  change sl2 with (nil ++ sl2). apply S. econstructor; eauto.
...
- (* seqor true *)
  ...
  econstructor; split.
  left. eapply plus_left. constructor.
  apply step_makeif with (b := Int.one) (v1 := v); auto. 
  setoid_rewrite <- H1. congruence.
  reflexivity. 
  eapply match_exprstates; eauto.
  change sl2 with (nil ++ sl2). apply S. econstructor; eauto.
...
- (* condition *)
  ...
  exploit tr_simple_rvalue; eauto. intros [SL [TY EV]].
  subst sl0; simpl Kseqlist. 
  destruct (negb (Int.eq b Int.zero)) eqn:?.
  econstructor; split.
  left. eapply plus_left. constructor.
  eapply star_trans. apply step_makeif with (b := b) (v1 := v); auto. 
  setoid_rewrite <- H1. congruence.
  rewrite Heqb0. apply push_seq. reflexivity. reflexivity.
  rewrite <- Kseqlist_app.
  eapply match_exprstates; eauto.
  ...
  + (* for effects *)
  exploit tr_simple_rvalue; eauto. intros [SL [TY EV]].
  subst sl0; simpl Kseqlist. destr_cond_match.
  econstructor; split.
  left. eapply plus_left. constructor.
  eapply star_trans. apply step_makeif with (b := b) (v1 := v); auto. congruence.
  rewrite Heqb0. apply push_seq. 
  reflexivity. traceEq.
  rewrite <- Kseqlist_app.
...
Lemma sstep_simulation:
  forall S1 t S2, Csem.sstep ge needed_stackspace S1 t S2 ->
  forall S1' (MS: match_states S1 S1'),
  exists S2',
     (plus (fun ge => step1 ge needed_stackspace) tge S1' t S2' \/
       (star (fun ge => step1 ge needed_stackspace) tge S1' t S2' /\ measure S2 < measure S1)%nat)
...
- (* skip seq *)
  ...
  econstructor; eauto.
- (* continue seq *)
  inv H7; inv H6. econstructor; split.
  left. apply plus_one; constructor.
  econstructor; eauto. constructor.
- (* break seq *)
  inv H7; inv H6. econstructor; split.
...
- (* ifthenelse *)
  ...
  apply step_ifthenelse with (v1 := v) (b := bb); auto. traceEq.
  destr_cond_match; econstructor; eauto.
...
- (* while false *)
  ...
  eapply star_trans. apply step_makeif with (v1 := v) (b := Int.zero); auto.
...
- (* while true *)
  ...
  econstructor; split.
  left. simpl. eapply plus_left. constructor.
  eapply star_right. apply step_makeif with (v1 := v) (b := Int.one); auto.
  constructor.
  reflexivity. traceEq.
...
- (* dowhile false *)
  ...
  econstructor; split.
  left. simpl. eapply plus_left. constructor. 
  eapply star_right. apply step_makeif with (v1 := v) (b := Int.zero); auto. 
  constructor. 
  reflexivity. traceEq.
  constructor; auto. constructor.
- (* dowhile true *)
...
- (* return none *)
  ...
  left. apply plus_one. econstructor; eauto. inv H7. rewrite  H2; eauto.
...
- (* return some 2 *)
  ...
  eauto. inv H8. rewrite H3; eauto. traceEq.
  constructor. apply match_cont_call; auto.
- (* skip return *)
  inv H9. 
  assert (is_call_cont tk). inv H10; simpl in *; auto.
  econstructor; split.
  left. apply plus_one. eapply step_skip_call; eauto.
  inv H8. rewrite H4; eauto.
  constructor. auto.
...
- (* internal function *)
  ...
  + left; apply plus_one. eapply step_internal_function. econstructor.
    * rewrite H8, H10; auto.
    * rewrite H10, H8; auto. eapply alloc_variables_preserved; eauto.
    * eauto.
    * rewrite H5; eauto.
...
Theorem simulation:
  forall S1 t S2, Cstrategy.step ge needed_stackspace S1 t S2 ->
  forall S1' (MS: match_states S1 S1'),
  exists S2',
     (plus (fun ge => step1 ge needed_stackspace) tge S1' t S2' \/
       (star (fun ge => step1 ge needed_stackspace) tge S1' t S2' /\ measure S2 < measure S1)%nat)
...
Lemma transl_initial_states:
  forall sg S,
  Csem.initial_state prog sg S ->
  exists S', Clight.initial_state tprog sg S' /\ match_states S S'.
Proof.
  intros. inv H. 
  ...
  - econstructor.
    + exploit Genv.init_mem_match; eauto.  
      unfold Csem.fid, fid.
      intros a b0 TR'; inv TR'. inv H. congruence. auto.
...
Theorem transl_program_correct:
  forall sg, 
    forward_simulation
      (Cstrategy.semantics prog needed_stackspace sg)
      (Clight.semantics1 tprog needed_stackspace sg).
Proof.
  intro sg.
