...
Definition block_alignment sz :=
  two_power_nat (Alloc.alignment_of_size sz).
...
Definition build_compilenv (f: Csharpminor.function) : compilenv * Z :=
  assign_variables (PTree.empty Z, 0) (varsort' (Csharpminor.fn_vars f)).
...
Definition transl_funbody
      (cenv: compilenv) (stacksize: Z) (sspos: 0 <= stacksize) (f: Csharpminor.function): res function :=
...
Lemma assign_variable_pos:
  forall l t z (Zpos: 0 <= z),
    0 <= snd (assign_variable (t, z) l).
Proof.
  intros; simpl.
  unfold assign_variable. destruct l.
  simpl.
  generalize (align_le z _ (block_alignment_pos z0)).
  Psatz.lia.
Qed.

Lemma assign_variables_pos:
  forall l t z (Zpos: 0 <= z),
    0 <= snd (assign_variables (t, z) l).
Proof.
  induction l; simpl; intros; eauto.
  destruct (assign_variable (t,z) a) eqn:?.
  apply IHl.
  generalize (assign_variable_pos a t z). rewrite Heqp; simpl.
  auto.
Qed.

Lemma build_compilenv_size_pos:
  forall f, 0 <= snd (build_compilenv f).
Proof.
  unfold build_compilenv; intros.
  apply assign_variables_pos; auto. omega.
Qed.

Definition transl_function (f: Csharpminor.function): res function :=
  let cs := build_compilenv f in
  let cenv := fst cs in
  let stacksize := snd cs in
  if zle (align stacksize (two_power_nat Memory.MA)) Int.max_unsigned
  then transl_funbody cenv stacksize (build_compilenv_size_pos _) f
