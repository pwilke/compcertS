...
Definition do_cast (v: expr_sym) (t1 t2: type) : res expr_sym :=
  match sem_cast_expr v t1 t2 with
  | Some v' => OK v'
  | None => Error(msg "undefined cast")
  end.

Fixpoint constval (a: expr) : res expr_sym :=
  match a with
  | Csyntax.Eval v ty =>
      match v with
      | Eval (Vint _) | Eval (Vfloat _) |Eval (Vsingle _) | Eval (Vlong _) => OK v
      | Eval (Vptr _ _) | Eval Vundef | _ => Error(msg "illegal constant")
      end
  | Evalof l ty =>
  ...
  | Csyntax.Eunop op r1 ty =>
      do v1 <- constval r1;
      OK (sem_unary_operation_expr op v1 (typeof r1))
  | Csyntax.Ebinop op r1 r2 ty =>
      do v1 <- constval r1;
      do v2 <- constval r2;
      match sem_binary_operation_expr op v1 (typeof r1) v2 (typeof r2) with
      | Some v => OK v
  ...
  | Esizeof ty1 ty =>
      OK (Eval (Vint (Int.repr (sizeof ty1))))
  | Ealignof ty1 ty =>
      OK (Eval (Vint (Int.repr (alignof ty1))))
  | Eseqand r1 r2 ty =>
      do v1 <- constval r1;
      do v2 <- constval r2;
      match Mem.mem_norm Mem.empty (bool_expr v1 (typeof r1)) with
          Vint b =>
          if negb (Int.eq b Int.zero)
          then do v3 <- do_cast v2 (typeof r2) type_bool; do_cast v3 type_bool ty
          else OK (Eval (Vint Int.zero))
        | _ => Error(msg "undefined && operation")
      end
  | Eseqor r1 r2 ty =>
      do v1 <- constval r1;
      do v2 <- constval r2;
      match Mem.mem_norm Mem.empty (bool_expr v1 (typeof r1)) with
          Vint b =>
          if (Int.eq b Int.zero)
          then do v3 <- do_cast v2 (typeof r2) type_bool; do_cast v3 type_bool ty
          else OK (Eval (Vint Int.one))
        | _ => Error(msg "undefined || operation")
      end
  | Econdition r1 r2 r3 ty =>
      ...
      match Mem.mem_norm Mem.empty (bool_expr v1 (typeof r1)) with
          Vint b =>
          if negb (Int.eq b Int.zero)
          then do_cast v2 (typeof r2) ty
          else do_cast v3 (typeof r3) ty
        | _ => Error(msg "condition is undefined")
      end
  | Ecomma r1 r2 ty =>
      do v1 <- constval r1; constval r2
  | Evar x ty =>
      OK(Eval(Vptr x Int.zero))
  | Ederef r ty =>
      constval r
  | Efield l f ty =>
      match typeof l with
      | Tstruct id fList _ =>
          ...
          do v <- constval l;
          OK (Val.add v (Eval (Vint (Int.repr delta))))
      | Tunion id fList _ =>
          constval l
      | _ =>
...
Fixpoint eval (e:expr_sym) : val :=
  match e with
  | Eval v => v
  | Eundef x x0 => Vundef
  | Eunop u t e => match u with
                    (OpSignext n|OpZeroext n) =>
                    if zeq n 32
                    then eval e
                    else ExprEval.fun_of_unop u t (eval e)
                  | _ => ExprEval.fun_of_unop u t (eval e)
                  end
  | Ebinop b t t' e e' => ExprEval.fun_of_binop b t t' (eval e) (eval e')
  end.
...
Definition transl_init_single (ty: type) (a: expr) : res init_data :=
  ...
  match eval v2, ty with
  | Vint n, Ctypes.Tint (I8|IBool) sg _ => OK(Init_int8 n)
  | Vint n, Ctypes.Tint I16 sg _ => OK(Init_int16 n)
  | Vint n, Ctypes.Tint I32 sg _ => OK(Init_int32 n)
  | Vint n, Tpointer _ _ => OK(Init_int32 n)
  | Vint n, Tcomp_ptr _ _ => OK(Init_int32 n)
  | Vlong n, Ctypes.Tlong _ _ => OK(Init_int64 n)
  | Vsingle f, Ctypes.Tfloat F32 _ => OK(Init_float32 f)
  | Vfloat f, Ctypes.Tfloat F64 _ => OK(Init_float64 f)
  | Vptr id ofs, Ctypes.Tint I32 sg _ => OK(Init_addrof id ofs)
...
Definition padding (frm to: Z) : list init_data :=
  if zlt frm to then Init_space (to - frm) O :: nil else nil.
...
with transl_init_array (ty: type) (il: initializer_list) (sz: Z)
  ...
  | Init_nil =>
      ...
      else if zle 0 sz then OK (Init_space (sz * sizeof ty) O :: nil)
