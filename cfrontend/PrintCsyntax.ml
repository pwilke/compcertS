...
let rec precedence = function
  ...
  | Eaddrof _ -> (15, RtoL)
  | Ecast _ -> (14, RtoL)
  | Csyntax.Ebinop((Omul|Odiv|Omod), _, _, _) -> (13, LtoR)
  | Csyntax.Ebinop((Oadd|Osub), _, _, _) -> (12, LtoR)
  | Csyntax.Ebinop((Oshl|Oshr), _, _, _) -> (11, LtoR)
  | Csyntax.Ebinop((Olt|Ogt|Ole|Oge), _, _, _) -> (10, LtoR)
  | Csyntax.Ebinop((Oeq|One), _, _, _) -> (9, LtoR)
  | Csyntax.Ebinop(Oand, _, _, _) -> (8, LtoR)
  | Csyntax.Ebinop(Oxor, _, _, _) -> (7, LtoR)
  | Csyntax.Ebinop(Oor, _, _, _) -> (6, LtoR)
  | Eseqand _ -> (5, LtoR)
  | Eseqor _ -> (4, LtoR)
  | Econdition _ -> (3, RtoL)
  | Eassign _ -> (2, RtoL)
...
let print_pointer_hook
   : (formatter -> Values_symbolictype.expr_sym -> unit) ref
   = ref (fun p (bofs) -> ())
...
let print_typed_value p v ty =
  ...
  | Vptr(b, ofs), _ ->
      fprintf p "<ptr%a>" !print_pointer_hook (Eval (Vptr (b, ofs)))
  | Vundef, _ ->
      fprintf p "<undef>"
...
let print_expr_symb_typed p e ty =
  fprintf p "%s" (PrintValues.print_expr_symb_typed e ty)

let print_expr_symb p e  =
  fprintf p "%s" (PrintValues.print_expr_symb e)
...
let rec expr p (prec, e) =
  ...
  begin match e with
  | Eloc(bofs, _) ->
      fprintf p "<loc%a>" !print_pointer_hook bofs
  | Evar(id, _) ->
...
let print_init p = function
  ...
  | Init_space (n,_) -> fprintf p "/* skip %ld */@ " (camlint_of_coqint n)
...
let rec collect_expr e =
  ...
  | Csyntax.Eunop(_, r, _) -> collect_expr r
  | Csyntax.Ebinop(_, r1, r2, _) -> collect_expr r1; collect_expr r2
