...
let make_builtin_memcpy args =
  ...
  | Econs(dst, Econs(src, Econs(sz, Econs(al, Enil)))) ->
      let sz1 =
        ...
        | Errors.OK(Values_symbolictype.Eval (Vint n)) -> n
...
let ezero = Eval(Values_symbolictype.Eval(Vint(coqint_of_camlint 0l)), type_int32s)
...
let rec convertExpr env e =
  ...
  | C.EConst(C.CInt(i, (ILongLong|IULongLong), _)) ->
      Eval(Values_symbolictype.Eval(Vlong(coqint_of_camlint64 i)), ty)
  | C.EConst(C.CInt(i, k, _)) ->
      Eval(Values_symbolictype.Eval(Vint(convertInt i)), ty)
...
let convertFundef loc env fd =
  ...
  Hashtbl.add decl_atom id'
    { a_storage = fd.fd_storage;
      ...
      a_loc = loc };
  (id', Gfun(Internal {
		 fn_id = Camlcoq.P.of_int (fd.fd_name.stamp);
		 fn_return = ret;
...
let convertGlobvar loc env (sto, id, ty, optinit) =
  ...
  let init' =
    match optinit with
    | None ->
        if sto = C.Storage_extern then [] else [Init_space (sz,Nat.of_int 0)]
    | Some i ->
