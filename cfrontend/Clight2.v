...
Definition try_norm m e := max_def (Mem.mem_norm m e) e.
...
Inductive deref_loc (ty: type) (m: mem) (ptr: expr_sym) : expr_sym -> Prop :=
  | deref_loc_value: forall chunk v
                       ...
                       (LOAD: Mem.loadv chunk m ptr = Some v),
      deref_loc ty m ptr (try_norm m v)
  | deref_loc_reference:
      forall 
        (AM: access_mode ty = By_reference),
        deref_loc ty m ptr (try_norm m ptr)
  | deref_loc_copy:
      forall 
...
| assign_loc_value:
    forall v chunk m'
      (AM: access_mode ty = By_value chunk)
      (STORE: Mem.storev chunk m addr (try_norm m v) = Some m'),
      assign_loc ty m addr v m'
| assign_loc_copy:
    forall b ofs b' ofs' bytes m' v
      (AM: access_mode ty = By_copy)
      (AL': sizeof ty > 0 -> (alignof_blockcopy ty | Int.unsigned ofs'))
      (AL: sizeof ty > 0 -> (alignof_blockcopy ty | Int.unsigned ofs))
      ...
      (NORM': sizeof ty > 0 -> Mem.mem_norm m v = Vptr b' ofs')
      (NORM: sizeof ty > 0 -> Mem.mem_norm m addr = Vptr b ofs),
      assign_loc ty m addr v m'.
...
Inductive alloc_variables: env -> mem ->
  ...
  | alloc_variables_cons:
      forall e m id ty vars m1 b1 m2 e2
        (ALLOC: Mem.alloc m 0 (sizeof ty) Normal  = Some (m1, b1))
        (AV: alloc_variables (PTree.set id (b1, ty) e) m1 vars e2 m2),
...
Inductive bind_parameters (e: env):
  ...
  | bind_parameters_cons:
      forall m id ty params v1 vl b m1 m2
        (GET: PTree.get id e = Some(b, ty))
        (AL: assign_loc ty m (Eval (Vptr b Int.zero)) v1 m1)
        (BP: bind_parameters e m1 params vl m2),
...
Inductive eval_expr: expr -> expr_sym -> Prop :=
  ...
  | eval_Eunop:  forall op a ty v1 v,
      ...
      eval_expr (Eunop op a ty) (try_norm m v)
  | eval_Ebinop: forall op a1 a2 ty v1 v2 v,
      eval_expr a1 v1 ->
      eval_expr a2 v2 ->
      sem_binary_operation_expr op v1 (typeof a1) v2 (typeof a2) = Some v ->
      eval_expr (Ebinop op a1 a2 ty) (try_norm m v)
  | eval_Ecast:   forall a ty v1 v,
      eval_expr a v1 ->
      sem_cast_expr v1 (typeof a) ty = Some v ->
      eval_expr (Ecast a ty) (try_norm m v)
...
with eval_lvalue: expr -> expr_sym  -> Prop :=
     ...
     | eval_Efield_struct:
         forall a i ty id fList att delta v,
           ...
           eval_lvalue (Efield a i ty) (try_norm m (Val.add v (Eval (Vint (Int.repr delta)))))
     | eval_Efield_union:
         forall a i ty id fList att v,
           eval_expr a v ->
           typeof a = Tunion id fList att ->
           eval_lvalue (Efield a i ty) (try_norm m v).
...
Inductive eval_exprlist: list expr -> typelist -> list expr_sym -> Prop :=
  ...
  | eval_Econs:   forall a bl ty tyl v1 v2 vl,
      ...
      eval_exprlist (a :: bl) (Tcons ty tyl) (try_norm m v2 :: vl).
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_assign:   forall f a1 a2 k e le m v2 v m' vptr,
      ...
      assign_loc (typeof a1) m vptr (try_norm m v) m' ->
  ...
  | step_set:   forall f id a k e le m v,
      ...
      step (State f (Sset id a) k e le m)
        E0 (State f Sskip k e (PTree.set id (try_norm m v) le) m)
  ...
  | step_builtin:   forall f optid ef tyargs al k e le m vargs t vres m',
      eval_exprlist e le m al tyargs vargs ->
      external_call ef ge (map (try_norm m) vargs) m t vres m' ->
      step (State f (Sbuiltin optid ef tyargs al) k e le m)
         t (State f Sskip k e (set_opttemp optid (try_norm m vres) le) m')
  ...
  | step_return_0: forall f k e le m m' m'',
      ...
      MemReserve.release_boxes m' (needed_stackspace (fn_id f)) = Some m'' ->
  ...
  | step_return_1: forall f a k e le m v v' m' m'',
      ...
      step (State f (Sreturn (Some a)) k e le m)
        E0 (Returnstate (try_norm m v') (call_cont k) m'')
  ...
  | step_returnstate: forall v optid f e le k m,
      step (Returnstate v (Kcall optid f e le k) m)
        E0 (State f Sskip k e (set_opttemp optid (try_norm m v) le) m).
...
Inductive function_entry1 needed_stackspace  (f: function) (vargs: list expr_sym) (m: mem) (e: env) (le: temp_env) (m': mem) : Prop :=
  | function_entry1_intro: forall m1 m2,
      ...
      MemReserve.reserve_boxes m1 (needed_stackspace f.(fn_id)) = Some m2 ->
      bind_parameters e m2 f.(fn_params) (map (try_norm m2) vargs) m' ->
...
| function_entry2_intro:
    forall m2,
      ...
      MemReserve.reserve_boxes m2 (needed_stackspace f.(fn_id)) = Some m' ->
