...
Fixpoint simple (a: expr) : bool :=
  ...
  | Csyntax.Eunop _ r1 _ => simple r1
  | Csyntax.Ebinop _ r1 r2 _ => simple r1 && simple r2
...
Inductive eval_simple_lvalue: expr -> expr_sym -> Prop :=
| esl_loc: forall ptr ty,
             eval_simple_lvalue (Eloc ptr ty) ptr
| esl_var_local: forall x ty b,
                   e!x = Some(b, ty) ->
                   eval_simple_lvalue (Evar x ty) (Eval (Vptr b Int.zero))
| esl_var_global: forall x ty b,
                    e!x = None ->
                    Genv.find_symbol ge x = Some b ->
                    eval_simple_lvalue (Evar x ty) (Eval (Vptr b Int.zero))
| esl_deref: forall r ty  v,
               eval_simple_rvalue r v ->
               eval_simple_lvalue (Ederef r ty) v
| esl_field_struct: forall r f ty id fList a delta v,
                      eval_simple_rvalue r v ->
                      typeof r = Tstruct id fList a -> field_offset f fList = OK delta ->
                      eval_simple_lvalue (Efield r f ty) (Val.add v (Eval (Vint (Int.repr delta))))
| esl_field_union: forall r f ty id fList a v,
                     eval_simple_rvalue r v ->
                     typeof r = Tunion id fList a ->
                     eval_simple_lvalue (Efield r f ty) v

with eval_simple_rvalue: expr -> expr_sym -> Prop :=
  | esr_val: forall v ty,
      eval_simple_rvalue (Csyntax.Eval v ty) v
  | esr_rvalof: forall ptr l ty v ,
      eval_simple_lvalue l ptr ->
      ty = typeof l -> type_is_volatile ty = false ->
      deref_loc ge ty m ptr  E0 v ->
      eval_simple_rvalue (Evalof l ty) v
  | esr_addrof: forall ptr l ty,
      eval_simple_lvalue l ptr ->
      eval_simple_rvalue (Eaddrof l ty) ptr
  | esr_unop: forall op r1 ty v1 v,
      eval_simple_rvalue r1 v1 ->
      sem_unary_operation_expr op v1 (typeof r1) =  v ->
      eval_simple_rvalue (Csyntax.Eunop op r1 ty) v
  | esr_binop: forall op r1 r2 ty v1 v2 v,
      eval_simple_rvalue r1 v1 -> eval_simple_rvalue r2 v2 ->
      sem_binary_operation_expr op v1 (typeof r1) v2 (typeof r2) = Some v ->
      eval_simple_rvalue (Csyntax.Ebinop op r1 r2 ty) v
  | esr_cast: forall ty r1 v1 v,
      eval_simple_rvalue r1 v1 ->
      sem_cast_expr v1 (typeof r1) ty = Some v ->
      eval_simple_rvalue (Ecast r1 ty) v
  | esr_sizeof: forall ty1 ty,
      eval_simple_rvalue (Esizeof ty1 ty) (Eval (Vint (Int.repr (sizeof ty1))))
  | esr_alignof: forall ty1 ty,
      eval_simple_rvalue (Ealignof ty1 ty) (Eval (Vint (Int.repr (alignof ty1)))).

Inductive eval_simple_list: exprlist -> typelist -> list expr_sym -> Prop :=
  | esrl_nil:
      eval_simple_list Enil Tnil nil
  | esrl_cons: forall r rl ty tyl v vl v',
      eval_simple_rvalue r v' -> sem_cast_expr v' (typeof r) ty = Some v ->
...
Inductive leftcontext: kind -> kind -> (expr -> expr) -> Prop :=
  ...
  | lctx_unop: forall k C op ty,
      leftcontext k RV C -> leftcontext k RV (fun x => Csyntax.Eunop op (C x) ty)
  | lctx_binop_left: forall k C op e2 ty,
      leftcontext k RV C -> leftcontext k RV (fun x => Csyntax.Ebinop op (C x) e2 ty)
  | lctx_binop_right: forall k C op e1 ty,
      simple e1 = true -> leftcontext k RV C ->
      leftcontext k RV (fun x => Csyntax.Ebinop op e1 (C x) ty)
...
Inductive estep: state -> trace -> state -> Prop :=
  ...
  | step_expr: forall f r k e m v ty,
      ...
      match r with Csyntax.Eval _ _ => False | _ => True end ->
      ty = typeof r ->
      estep (ExprState f r k e m)
         E0 (ExprState f (Csyntax.Eval v ty) k e m)

  | step_rvalof_volatile: forall f C l ty k e m ptr t v ,
      leftcontext RV RV C ->
      ...
      estep (ExprState f (C (Evalof l ty)) k e m)
          t (ExprState f (C (Csyntax.Eval v ty)) k e m)

  | step_seqand_true: forall f C r1 r2 ty k e m v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m r1 v ->
      Mem.mem_norm m (bool_expr v (typeof r1)) = Vtrue ->
  ...
  | step_seqand_false: forall f C r1 r2 ty k e m v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m r1 v ->
      Mem.mem_norm m (bool_expr v (typeof r1)) = Vfalse ->
      estep (ExprState f (C (Eseqand r1 r2 ty)) k e m)
         E0 (ExprState f (C (Csyntax.Eval (Eval (Vint Int.zero)) ty)) k e m)
  ...
  | step_seqor_true: forall f C r1 r2 ty k e m v,
      ...
      estep (ExprState f (C (Eseqor r1 r2 ty)) k e m)
         E0 (ExprState f (C (Csyntax.Eval (Eval (Vint Int.one)) ty)) k e m)
  ...
  | step_condition: forall f C r1 r2 r3 ty k e m v b,
      leftcontext RV RV C ->
      eval_simple_rvalue e m r1 v ->
      Mem.mem_norm m (bool_expr v (typeof r1)) = Vint b ->
      estep (ExprState f (C (Econdition r1 r2 r3 ty)) k e m)
         E0 (ExprState f (C (Eparen (if negb (Int.eq b Int.zero) then r2 else r3) ty)) k e m)

  | step_assign: forall f C l r ty k e m ptr v v' t m' ,
      leftcontext RV RV C ->
      eval_simple_lvalue e m l ptr ->
      eval_simple_rvalue e m r v ->
      sem_cast_expr v (typeof r) (typeof l) = Some v' ->
      assign_loc ge (typeof l) m ptr  v' t m' ->
      ty = typeof l ->
      estep (ExprState f (C (Eassign l r ty)) k e m)
          t (ExprState f (C (Csyntax.Eval v' ty)) k e m')

  | step_assignop: forall f C op l r tyres ty k e m ptr v1 v2 v3 v4 t1 t2 m' t ,
      leftcontext RV RV C ->
      eval_simple_lvalue e m l ptr ->
      deref_loc ge (typeof l) m ptr t1 v1 ->
      eval_simple_rvalue e m r v2 ->
      sem_binary_operation_expr op v1 (typeof l) v2 (typeof r) = Some v3 ->
      sem_cast_expr v3 tyres (typeof l) = Some v4 ->
      assign_loc ge (typeof l) m ptr  v4 t2 m' ->
      ty = typeof l ->
      t = t1 ** t2 ->
      estep (ExprState f (C (Eassignop op l r tyres ty)) k e m)
          t (ExprState f (C (Csyntax.Eval v4 ty)) k e m')

  | step_assignop_stuck: forall f C op l r tyres ty k e m ptr v1 v2 t ,
      leftcontext RV RV C ->
      eval_simple_lvalue e m l ptr ->
      deref_loc ge (typeof l) m ptr  t v1 ->
      eval_simple_rvalue e m r v2 ->
      match sem_binary_operation_expr op v1 (typeof l) v2 (typeof r) with
      | None => True
      | Some v3 =>
          match sem_cast_expr v3 tyres (typeof l) with
          | None => True
          | Some v4 => forall t2 m', ~(assign_loc ge (typeof l) m ptr v4 t2 m')
          end
      end ->
      ty = typeof l ->
      estep (ExprState f (C (Eassignop op l r tyres ty)) k e m)
  ...
  | step_postincr: forall f C id l ty k e m ptr v1 v2 v3 t1 t2 m' t ,
      leftcontext RV RV C ->
      eval_simple_lvalue e m l ptr ->
      deref_loc ge ty m ptr t1 v1 ->
      sem_incrdecr_expr id v1 ty = Some v2 ->
      sem_cast_expr v2 (incrdecr_type ty) ty = Some v3 ->
      assign_loc ge ty m ptr v3 t2 m' ->
      ty = typeof l ->
      t = t1 ** t2 ->
      estep (ExprState f (C (Epostincr id l ty)) k e m)
          t (ExprState f (C (Csyntax.Eval v1 ty)) k e m')

  | step_postincr_stuck: forall f C id l ty k e m ptr v1 t ,
      leftcontext RV RV C ->
      eval_simple_lvalue e m l ptr ->
      deref_loc ge ty m ptr t v1 ->
      match sem_incrdecr_expr id v1 ty with
          Some v2 =>
          match sem_cast_expr v2 (incrdecr_type ty) ty with
          | None => True
          | Some v3 => forall t2 m', ~(assign_loc ge (typeof l) m ptr v3 t2 m')
          end
        | _ => True
      end ->
      ty = typeof l ->
  ...
  | step_paren: forall f C r ty k e m v1 v,
      ...
      sem_cast_expr v1 (typeof r) ty = Some v ->
  ...
  | step_builtin: forall f C ef tyargs rargs ty k e m vargs t vres m',
      ...
      estep (ExprState f (C (Ebuiltin ef tyargs rargs ty)) k e m)
          t (ExprState f (C (Csyntax.Eval vres ty)) k e m').

Definition step (S: state) (t: trace) (S': state) : Prop :=
  estep S t S' \/ sstep ge needed_stackspace S t S'.
...
Definition safe (s: Csem.state) : Prop :=
  forall s', star (fun ge => Csem.step ge needed_stackspace) ge s E0 s' ->
  (exists r, final_state s' r) \/ (exists t, exists s'', (fun ge => Csem.step ge needed_stackspace) ge s' t s'').

Lemma safe_steps:
  forall s s',
  safe s -> star (fun ge => Csem.step ge needed_stackspace) ge s E0 s' -> safe s'.
Proof.
  intros; red; intros. 
...
Lemma star_safe:
  forall s1 s2 t s3,
    safe s1 -> star (fun ge => Csem.step ge needed_stackspace) ge s1 E0 s2 ->
    (safe s2 -> star (fun ge => Csem.step ge needed_stackspace) ge s2 t s3) ->
  star (fun ge => Csem.step ge needed_stackspace) ge s1 t s3.
Proof.
...
Lemma plus_safe:
  forall s1 s2 t s3,
  safe s1 -> star (fun ge => Csem.step ge needed_stackspace) ge s1 E0 s2 -> (safe s2 -> plus (fun ge => Csem.step ge needed_stackspace) ge s2 t s3) ->
  plus (fun ge => Csem.step ge needed_stackspace) ge s1 t s3.
Proof.
...
Lemma callred_kind:
  forall m a fd args ty, callred ge m a fd args ty -> expr_kind a = RV.
Proof.
  induction 1; auto.
Qed.
...
Definition invert_expr_prop (a: expr) (m: mem) : Prop :=
  ...
  | Csyntax.Evalof (Eloc ptr ty') ty =>
      ty' = ty /\ exists t, exists v, deref_loc ge ty m ptr t v
  ...
  | Eseqand (Csyntax.Eval v1 ty1) r2 ty =>
      exists b, Mem.mem_norm m (bool_expr v1 ty1) = Values.Val.of_bool b
  ...
  | Eassign (Eloc ptr ty1) (Csyntax.Eval v2 ty2) ty =>
      ...
      ty = ty1 /\ sem_cast_expr v2 ty2 ty1 = Some v /\ assign_loc ge ty1 m ptr  v t m'
...
Proof.
  ...
  exists v; auto.
  exists v; auto.
  exists true; subst; auto. exists false; subst; auto.
  exists true; subst; auto. exists false; subst; auto.
  exists b; subst; auto.
...
Lemma eval_simple_steps:
  (forall a v,
     ...
     forall C,
       context RV RV C ->
       star (fun ge => Csem.step ge needed_stackspace) ge (ExprState f (C a) k e m)
            E0 (ExprState f (C (Csyntax.Eval v (typeof a))) k e m))
  /\ (forall a ptr,
        eval_simple_lvalue e m a ptr ->
        forall C,
          context LV RV C ->
          star (fun ge => Csem.step ge needed_stackspace) ge (ExprState f (C a) k e m)
               E0 (ExprState f (C (Eloc ptr (typeof a))) k e m)).
Proof.
...
- (* val *)
  apply star_refl.
- (* valof *)
  Steps H0 (fun x => C(Csyntax.Evalof x ty)).
  rewrite <- H1 in *. FinishR.
- (* addrof *)
  Steps H0 (fun x => C(Eaddrof x ty)). FinishR.
- (* unop *)
  Steps H0 (fun x => C(Csyntax.Eunop op x ty)). FinishR.
- (* binop *)
  Steps H0 (fun x => C(Csyntax.Ebinop op x r2 ty)).
  Steps H2 (fun x => C(Csyntax.Ebinop op (Csyntax.Eval v1 (typeof r1)) x ty)).
  FinishR.
- (* cast *)
...
Lemma eval_simple_lvalue_steps:
  forall a ptr, eval_simple_lvalue e m a ptr ->
...
Corollary eval_simple_rvalue_safe:
  ...
  safe (ExprState f (C (Csyntax.Eval v (typeof a))) k e m).
Proof.
...
Corollary eval_simple_lvalue_safe:
  ...
  safe (ExprState f (C (Eloc ptr (typeof a))) k e m).
Proof.
...
Lemma simple_can_eval:
  ...
  match from with
  | LV => exists ptr, eval_simple_lvalue e m a ptr
...
Ltac StepL REC C' a :=
  ...
  exploit (REC LV C'); eauto; intros [ptr E];
  assert (S: safe (ExprState f (C' (Eloc ptr (typeof a))) k e m)) by 
...
Ltac StepR REC C' a :=
  ...
  assert (S: safe (ExprState f (C' (Csyntax.Eval v (typeof a))) k e m)) by 
...
- (* addrof *)
  ...
  eexists; econstructor; eauto.
- (* unop *)
  StepR IHa (fun x => C(Csyntax.Eunop op x ty)) a.
  exploit safe_inv. eexact SAFE0. eauto. simpl. intros [v' EQ].
  eexists; econstructor; eauto.
- (* binop *)
  destruct (andb_prop _ _ S) as [S1 S2]; clear S.
  StepR IHa1 (fun x => C(Csyntax.Ebinop op x a2 ty)) a1.
  StepR IHa2 (fun x => C(Csyntax.Ebinop op (Csyntax.Eval v (typeof a1)) x ty)) a2.
...
Lemma simple_can_eval_rval:
  ...
  exists v, eval_simple_rvalue e m r v
        /\ safe (ExprState f (C (Csyntax.Eval v (typeof r))) k e m).
Proof.
...
Lemma simple_can_eval_lval:
  forall ll C, 
  simple ll = true -> context LV RV C -> safe (ExprState f (C ll) k e m) ->
  exists ptr, eval_simple_lvalue e m ll ptr
         /\ safe (ExprState f (C (Eloc ptr (typeof ll))) k e m).
Proof.
  intros. exploit (simple_can_eval ll LV); eauto. intros [ptr A]. 
  exists ptr; split; auto. eapply eval_simple_lvalue_safe; eauto.
Qed.

Fixpoint rval_list (vl: list expr_sym) (rl: exprlist) : exprlist :=
  match vl, rl with
  | v1 :: vl', Econs r1 rl' => Econs (Csyntax.Eval v1 (typeof r1)) (rval_list vl' rl')
  | _, _ => Enil
  end.

Inductive eval_simple_list': exprlist -> list expr_sym -> Prop :=
  | esrl'_nil:
...
Lemma eval_simple_list_steps:
  ...
  star (fun ge => Csem.step ge needed_stackspace) ge (ExprState f (C rl) k e m)
...
(* cons *)
  ...
  apply IHeval_simple_list' with (C := fun x => C(Econs (Csyntax.Eval v (typeof r)) x)); auto.
  auto.
Qed.
...
Proof.
  ...
  intros [v1 EV1].
  exploit (IHrl (fun x => C(Econs (Csyntax.Eval v1 (typeof r1)) x))); eauto.
...
Variable e: env.
Variable m: mem.
Variable l : list block.

Definition simple_side_effect (r: expr) : Prop :=
  match r with
  | Csyntax.Evalof l _ => simple l = true /\ type_is_volatile (typeof l) = true
...
(* rvalof *)
  Kind. Rec H LV C (fun x => Csyntax.Evalof x ty).
  destruct (type_is_volatile (typeof l0)) eqn:?.
...
(* unop *)
  Kind. Rec H RV C (fun x => Csyntax.Eunop op x ty).
(* binop *)
  Kind. Rec H RV C (fun x => Csyntax.Ebinop op x r2 ty). rewrite H3.
  Rec H0 RV C (fun x => Csyntax.Ebinop op r1 x ty).
(* cast *)
...
(* assign *)
  Kind. Rec H LV C (fun x => Eassign x r ty). Rec H0 RV C (fun x => Eassign l0 x ty). Base.
(* assignop *)
  Kind. Rec H LV C (fun x => Eassignop op x r tyres ty). Rec H0 RV C (fun x => Eassignop op l0 x tyres ty). Base.
(* postincr *)
...
Lemma estep_simulation:
  forall S t S',
  estep S t S' -> plus (fun ge => Csem.step ge needed_stackspace) ge S t S'.
Proof.
  intros. inv H. 
- (* simple *)
...
- (* valof volatile *)
  eapply plus_right. 
  eapply eval_simple_lvalue_steps with (C := fun x => C(Csyntax.Evalof x (typeof l))); eauto.
  left. apply step_rred; eauto. econstructor; eauto. auto.
- (* seqand true *)
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Eseqand x r2 ty)); eauto. 
  left. apply step_rred; eauto. eapply red_seqand_true; eauto. traceEq.
- (* seqand false *)
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Eseqand x r2 ty)); eauto. 
  left. apply step_rred; eauto. eapply red_seqand_false; auto. traceEq.
- (* seqor true *)
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Eseqor x r2 ty)); eauto. 
  left. apply step_rred; eauto. eapply red_seqor_true; auto. traceEq.
- (* seqor false *)
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Eseqor x r2 ty)); eauto. 
  left. apply step_rred; eauto. eapply red_seqor_false; auto. traceEq.
- (* condition *)
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Econdition x r2 r3 ty)); eauto.
  left; apply step_rred; eauto. econstructor; auto. auto. 
- (* assign *)
  eapply star_plus_trans.
  eapply eval_simple_lvalue_steps with (C := fun x => C(Eassign x r (typeof l))); eauto.
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Eassign (Eloc ptr (typeof l)) x (typeof l))); eauto.
...
- (* assignop *) 
  ...
  eapply star_plus_trans.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Eassignop op (Eloc ptr (typeof l)) x tyres (typeof l))); eauto.
  eapply plus_left.
  left; apply step_rred; auto. econstructor; eauto.
  eapply star_left.
  left; apply step_rred with (C := fun x => C(Eassign (Eloc ptr (typeof l)) x (typeof l))); eauto. econstructor; eauto.  
  apply star_one. 
  left; apply step_rred; auto. econstructor; eauto. 
...
- (* assignop stuck *)
  ...
  eapply plus_left.
  left; apply step_rred; auto. econstructor; eauto.
  destruct (sem_binary_operation_expr op v1 (typeof l) v2 (typeof r)) as [v3|] eqn:?.
  eapply star_left.
  left; apply step_rred with (C := fun x => C(Eassign (Eloc ptr (typeof l)) x (typeof l))); eauto. econstructor; eauto.  
  apply star_one. 
  ...
  reflexivity.
  apply star_one.
  left; eapply step_stuck with (C := fun x => C(Eassign (Eloc ptr (typeof l)) x (typeof l))); eauto.
...
- (* postincr *) 
  ...
  econstructor; eauto.
  eapply star_left.
  left; apply step_rred with (C := fun x => C (Ecomma (Eassign (Eloc ptr (typeof l)) x (typeof l)) (Csyntax.Eval v1 (typeof l)) (typeof l))); eauto.
  econstructor; eauto.
  destruct id; simpl; eauto.
  eapply star_left.
  left; apply step_rred with (C := fun x => C (Ecomma x (Csyntax.Eval v1 (typeof l)) (typeof l))); eauto.
  econstructor; eauto.
  apply star_one. 
  left; apply step_rred; auto. econstructor; eauto. 
...
- (* postincr stuck *)
  ...
  assert (SEM: sem_binary_operation_expr op v1 (typeof l) (Eval (Vint Int.one)) type_int32s =
               (sem_incrdecr_expr id v1 (typeof l))).
    destruct id; auto.
    destruct (sem_incrdecr_expr id v1 (typeof l)) eqn:?; try discriminate.
  eapply star_left.
  left; apply step_rred with (C := fun x => C (Ecomma (Eassign (Eloc ptr (typeof l)) x (typeof l)) (Csyntax.Eval v1 (typeof l)) (typeof l))); eauto.
  econstructor; eauto.
  apply star_one.
  left; eapply step_stuck with (C := fun x => C (Ecomma x (Csyntax.Eval v1 (typeof l)) (typeof l))); eauto.
  red; intros. exploit imm_safe_inv; eauto. simpl. intros [v3 [m' [t' [A [B D]]]]].
  rewrite B in H3. eelim H3; eauto.
  reflexivity.
  
  apply star_one.
  left; eapply step_stuck with (C := fun x => C (Ecomma (Eassign (Eloc ptr (typeof l)) x (typeof l)) (Csyntax.Eval v1 (typeof l)) (typeof l))); eauto.
...
- (* call *)
  ...
  eapply plus_right.
  eapply eval_simple_list_steps with (C := fun x => C(Ecall (Csyntax.Eval vf (typeof rf)) x ty)); eauto.
...
Lemma can_estep:
  forall f a k e m,
  safe (ExprState f a k e m) ->
  match a with Csyntax.Eval _ _ => False | _ => True end ->
...
- (* side effect *)
  ...
  + (* valof volatile *)
    destruct Q.
    exploit (simple_can_eval_lval f k e m b (fun x => C(Csyntax.Evalof x ty))); eauto.
    intros [ptr [E1 S1]].
...
+ (* assign *)
  ...
  intros [ptr [E1 S1]].
  exploit (simple_can_eval_rval f k e m b2 (fun x => C(Eassign (Eloc ptr (typeof b1)) x ty))); eauto.
  intros [v [E2 S2]].
...
+ (* assignop *)
  ...
  intros [ptr [E1 S1]].
  exploit (simple_can_eval_rval f k e m b2 (fun x => C(Eassignop op (Eloc ptr (typeof b1)) x tyres ty))); eauto.
  intros [v [E2 S2]].
  exploit safe_inv. eexact S2. eauto. simpl. intros [t1 [v1 [A B]]].
  destruct (sem_binary_operation_expr op v1 (typeof b1) v (typeof b2)) as [v3|] eqn:?.
  destruct (sem_cast_expr v3 tyres (typeof b1)) as [v4|] eqn:?.
  destruct (classic (exists t2, exists m', assign_loc ge (typeof b1) m ptr  v4 t2 m')).
...
+ (* postincr *)
  ...
  destruct (sem_incrdecr_expr id v1 ty) as [v2|] eqn:?; try discriminate.  
  destruct (sem_cast_expr v2 (incrdecr_type ty) ty) as [v3|] eqn:?.
  destruct (classic (exists t2, exists m', assign_loc ge ty m ptr  v3 t2 m')).
  ...
  intros; red; intros. elim H0. exists t2; exists m'; congruence.
  econstructor; econstructor; eapply step_postincr_stuck; eauto. 
  rewrite Heqo. rewrite Heqo0. auto.
  econstructor; econstructor; eapply step_postincr_stuck; eauto. 
  rewrite Heqo. auto.
...
+ (* call *)
  ...
  intros [vf [E1 S1]].
  pose (C' := fun x => C(Ecall (Csyntax.Eval vf (typeof b)) x ty)).
...
Theorem step_simulation:
  forall S1 t S2,
  step S1 t S2 -> plus (fun ge => Csem.step ge needed_stackspace) ge S1 t S2.
Proof.
  intros. inv H.
...
Remark deref_loc_trace:
  forall F V (ge: Genv.t F V) ty m ptr t v ,
  deref_loc ge ty m ptr t v ->
...
Remark deref_loc_receptive:
  forall F V (ge: Genv.t F V) ty m ptr ev1 t1 v ev2 ,
  deref_loc ge ty m ptr  (ev1 :: t1) v ->
  match_traces ge (ev1 :: nil) (ev2 :: nil) ->
  t1 = nil /\ exists v', deref_loc ge ty m ptr (ev2 :: nil) v'.
Proof.
  intros.
...
Remark assign_loc_trace:
  forall F V (ge: Genv.t F V) ty m bofs t v m' ,
...
Remark assign_loc_receptive:
  forall F V (ge: Genv.t F V) ty m bofs ev1 t1 v m' ev2 ,
  assign_loc ge ty m bofs v (ev1 :: t1) m' ->
  match_traces ge (ev1 :: nil) (ev2 :: nil) ->
  ev1 :: t1 = ev2 :: nil.
Proof.
  intros.
...
Lemma semantics_strongly_receptive: 
  forall p ns sg, strongly_receptive (semantics p ns sg).
Proof.
  ...
  - (* receptiveness *)
    ...
    + (* assignop *)
      ...
      destruct (sem_binary_operation_expr op v1' (typeof l) v2 (typeof r)) as [v3'|] eqn:?.
      destruct (sem_cast_expr v3' tyres (typeof l)) as [v4'|] eqn:?.
      destruct (classic (exists t2', exists m'', assign_loc (Genv.globalenv p) (typeof l) m ptr  v4' t2' m'')).
    ...
    + (* postincr *)
      ...
      destruct (sem_incrdecr_expr id v1' (typeof l)) as [v2'|] eqn:?.
      destruct (sem_cast_expr v2' (incrdecr_type (typeof l)) (typeof l)) as [v3'|] eqn:?.
      destruct (classic (exists t2', exists m'', assign_loc (Genv.globalenv p) (typeof l) m ptr  v3' t2' m'')).
      destruct H1 as [t2' [m'' P]]. 
      ...
      rewrite Heqo. rewrite Heqo0.
      intros; red; intros; elim H1. exists t0; exists m'0; auto.
      econstructor; econstructor. left; eapply step_postincr_stuck with (v1 := v1'); eauto. 
      rewrite Heqo. rewrite Heqo0. auto.
      econstructor; econstructor. left; eapply step_postincr_stuck with (v1 := v1'); eauto. 
      rewrite Heqo. auto.
    + (* postincr stuck *)
      ...
      econstructor; econstructor. left; eapply step_postincr_stuck with (v1 := v1'); eauto. 
      rewrite Heqo. rewrite Heqo0.
      intros; red; intros; elim H1. exists t2; exists m'; auto.
      econstructor; econstructor. left; eapply step_postincr_stuck with (v1 := v1'); eauto. 
      rewrite Heqo; rewrite Heqo0; auto.
      econstructor; econstructor. left; eapply step_postincr_stuck with (v1 := v1'); eauto. 
      rewrite Heqo; auto.
...
Theorem strategy_simulation:
  forall p ns sg, backward_simulation (Csem.semantics p ns sg) (semantics p ns sg).
Proof.
  intros.
...
Inductive outcome: Type :=
   ...
   | Out_return: option (expr_sym * type) -> outcome. (**r terminated by [return] *)
...
Definition outcome_result_value (out: outcome) (t: type) (v: expr_sym) : Prop :=
  match out, t with
  | Out_normal, Tvoid => v = Eval Vundef
  | Out_return None, Tvoid => v = Eval Vundef
  | Out_return (Some (v', ty')), ty => 
    ty <> Tvoid /\ sem_cast_expr v' ty' ty = Some v
  | _, _ => False
  end. 
...
Inductive eval_expression: env -> mem -> expr -> trace -> mem -> expr_sym -> Prop :=
...
with eval_expr: env -> mem -> kind -> expr -> trace -> mem -> expr -> Prop :=
  | eval_val: forall e m v ty,
      eval_expr e m RV (Csyntax.Eval v ty) E0 m (Csyntax.Eval v ty)
  ...
  | eval_valof: forall e m a t m' a' ty,
      ...
      eval_expr e m RV (Csyntax.Evalof a ty) t m' (Csyntax.Evalof a' ty)
  | eval_valof_volatile: forall e m a t1 m' a' ty ptr t2 v ,
      type_is_volatile (typeof a) = true ->
      eval_expr e m LV a t1 m' a' ->
      eval_simple_lvalue ge e m' a' ptr ->
      deref_loc ge (typeof a) m' ptr  t2 v ->
      ty = typeof a ->
      eval_expr e m RV (Csyntax.Evalof a ty) (t1 ** t2) m' (Csyntax.Eval v ty)
  ...
  | eval_unop: forall e m a t m' a' op ty,
      eval_expr e m RV a t m' a' ->
      eval_expr e m RV (Csyntax.Eunop op a ty) t m' (Csyntax.Eunop op a' ty)
  | eval_binop: forall e m a1 t1 m' a1' a2 t2 m'' a2' op ty,
      eval_expr e m RV a1 t1 m' a1' -> eval_expr e m' RV a2 t2 m'' a2' ->
      eval_expr e m RV (Csyntax.Ebinop op a1 a2 ty) (t1 ** t2) m'' (Csyntax.Ebinop op a1' a2' ty)
  ...
  | eval_seqand_true: forall e m a1 a2 ty t1 m' a1' v1 t2 m'' a2' v2 v' v,
      ...
      Mem.mem_norm m' (bool_expr v1 (typeof a1)) = Vtrue ->
      eval_expr e m' RV a2 t2 m'' a2' -> eval_simple_rvalue ge e m'' a2' v2 ->
      sem_cast_expr v2 (typeof a2) type_bool = Some v' ->
      sem_cast_expr v' type_bool ty = Some v ->
      eval_expr e m RV (Eseqand a1 a2 ty) (t1**t2) m'' (Csyntax.Eval v ty)
  | eval_seqand_false: forall e m a1 a2 ty t1 m' a1' v1,
      eval_expr e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e m' a1' v1 ->
      Mem.mem_norm m' (bool_expr v1 (typeof a1)) = Vfalse ->
      eval_expr e m RV (Eseqand a1 a2 ty) t1 m' (Csyntax.Eval (Eval (Vint Int.zero)) ty)
  | eval_seqor_false: forall e m a1 a2 ty t1 m' a1' v1 t2 m'' a2' v2 v' v,
      ...
      sem_cast_expr v2 (typeof a2) type_bool = Some v' ->
      sem_cast_expr v' type_bool ty = Some v ->
      eval_expr e m RV (Eseqor a1 a2 ty) (t1**t2) m'' (Csyntax.Eval v ty)
  | eval_seqor_true: forall e m a1 a2 ty t1 m' a1' v1,
      eval_expr e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e m' a1' v1 ->
      Mem.mem_norm m' (bool_expr v1 (typeof a1)) = Vtrue ->
      eval_expr e m RV (Eseqor a1 a2 ty) t1 m' (Csyntax.Eval (Eval (Vint Int.one)) ty)
  | eval_condition: forall e m a1 a2 a3 ty t1 m' a1' v1 t2 m'' a' v' b v,
      eval_expr e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e m' a1' v1 ->
      Mem.mem_norm m' (bool_expr v1 (typeof a1)) = Vint b ->
      eval_expr e m' RV (if negb (Int.eq b Int.zero) then a2 else a3) t2 m'' a' -> eval_simple_rvalue ge e m'' a' v' ->
      sem_cast_expr v' (typeof (if negb (Int.eq b Int.zero) then a2 else a3)) ty = Some v ->
      eval_expr e m RV (Econdition a1 a2 a3 ty) (t1**t2) m'' (Csyntax.Eval v ty)
  ...
  | eval_assign: forall e m l r ty t1 m1 l' t2 m2 r' ptr v v' t3 m3 ,
      eval_expr e m LV l t1 m1 l' -> eval_expr e m1 RV r t2 m2 r' ->
      eval_simple_lvalue ge e m2 l' ptr ->
      eval_simple_rvalue ge e m2 r' v ->
      sem_cast_expr v (typeof r) (typeof l) = Some v' ->
      assign_loc ge (typeof l) m2 ptr  v' t3 m3 ->
      ty = typeof l ->
      eval_expr e m RV (Eassign l r ty) (t1**t2**t3) m3 (Csyntax.Eval v' ty)
  | eval_assignop: forall e m op l r tyres ty t1 m1 l' t2 m2 r' ptr
                          v1 v2 v3 v4 t3 t4 m3 ,
      eval_expr e m LV l t1 m1 l' -> eval_expr e m1 RV r t2 m2 r' ->
      eval_simple_lvalue ge e m2 l' ptr ->
      deref_loc ge (typeof l) m2 ptr  t3 v1 ->
      eval_simple_rvalue ge e m2 r' v2 ->
      sem_binary_operation_expr op v1 (typeof l) v2 (typeof r) = Some v3 ->
      sem_cast_expr v3 tyres (typeof l) = Some v4 ->
      assign_loc ge (typeof l) m2 ptr  v4 t4 m3 ->
      ty = typeof l ->
      eval_expr e m RV (Eassignop op l r tyres ty) (t1**t2**t3**t4) m3 (Csyntax.Eval v4 ty)
  | eval_postincr: forall e m id l ty t1 m1 l' ptr v1 v2 v3 m2 t2 t3 ,
      eval_expr e m LV l t1 m1 l' ->
      eval_simple_lvalue ge e m1 l' ptr ->
      deref_loc ge ty m1 ptr  t2 v1 ->
      sem_incrdecr_expr id v1 ty = Some v2 ->
      sem_cast_expr v2 (incrdecr_type ty) ty = Some v3 ->
      assign_loc ge ty m1 ptr  v3 t3 m2 ->
      ty = typeof l ->
      eval_expr e m RV (Epostincr id l ty) (t1**t2**t3) m2 (Csyntax.Eval v1 ty)
  ...
  | eval_call: forall e m rf rargs ty t1 m1 rf' t2 m2 rargs' vf vargs
      ...
      classify_fun (typeof rf) = fun_case_f targs tres cconv ->
      Genv.find_funct m2 ge vf = Some fd ->
      type_of_fundef fd = Tfunction targs tres cconv ->
      eval_funcall m2 fd vargs t3 m3 vres ->
      eval_expr e m RV (Ecall rf rargs ty) (t1**t2**t3) m3 (Csyntax.Eval vres ty)
...
with exec_stmt: env -> mem -> statement -> trace -> mem -> outcome -> Prop :=
  ...
  | exec_Sifthenelse: forall e m a s1 s2 t1 m1 v1 t2 m2 b out,
      ...
      Mem.mem_norm m1 (bool_expr v1 (typeof a)) = Vint b ->
      exec_stmt e m1 (if negb (Int.eq b Int.zero) then s1 else s2) t2 m2 out ->
  ...
  | exec_Sdowhile_false: forall e m s a t1 m1 out1 t2 m2 v,
      ...
      Mem.mem_norm m2 (bool_expr v (typeof a)) = Vfalse ->
  ...
  | exec_Sdowhile_loop: forall e m s a t1 m1 out1 t2 m2 v t3 m3 out,
      ...
      out_normal_or_continue out1 ->
      eval_expression e m1 a t2 m2 v ->
      Mem.mem_norm m2 (bool_expr v (typeof a)) = Vtrue ->
  ...
  | exec_Sswitch:   forall e m a sl t1 m1 v n t2 m2 out,
      eval_expression e m a t1 m1 v ->
      sem_switch_arg_expr m1 v (typeof a) = Some n ->
...
with eval_funcall: mem -> fundef -> list expr_sym -> trace -> mem -> expr_sym -> Prop :=
  | eval_funcall_internal: forall m f vargs t e m1 m2 m2' m3 out vres m4 m5,
      ...
      release_boxes m4 (needed_stackspace f.(fn_id)) = Some m5 ->
...
Scheme eval_expression_mut := Induction for eval_expression Sort Prop
  with eval_expr_mut := Induction for eval_expr Sort Prop
  with eval_exprlist_mut := Induction for eval_exprlist Sort Prop
  with exec_stmt_mut := Induction for exec_stmt Sort Prop
  with eval_funcall_mut := Induction for eval_funcall Sort Prop.
...
CoInductive evalinf_expr: env -> mem -> kind -> expr -> traceinf -> Prop :=
  ...
  | evalinf_valof: forall e m a t ty,
      ...
      evalinf_expr e m RV (Csyntax.Evalof a ty) t
  ...
  | evalinf_condition_2: forall e m a1 a2 a3 ty t1 m' a1' v1 t2 b,
      eval_expr e m RV a1 t1 m' a1' -> eval_simple_rvalue ge e m' a1' v1 ->
      Mem.mem_norm m' (bool_expr v1 (typeof a1)) = Vint b ->
      evalinf_expr e m' RV (if negb (Int.eq b Int.zero) then a2 else a3) t2 ->
...
with execinf_stmt: env -> mem -> statement -> traceinf -> Prop :=
  ...
  | execinf_Sifthenelse: forall e m a s1 s2 t1 m1 v1 t2 b,
      eval_expression e m a t1 m1 v1 ->
      Mem.mem_norm m1 (bool_expr v1 (typeof a)) = Vint b ->
      execinf_stmt e m1 (if negb (Int.eq b Int.zero) then s1 else s2) t2 ->
...
with evalinf_funcall: mem -> fundef -> list expr_sym -> traceinf -> Prop :=
  | evalinf_funcall_internal: forall m f vargs t e m1 m2 m2',
      list_norepet (var_names f.(fn_params) ++ var_names f.(fn_vars)) ->
      alloc_variables empty_env m (VarSort.varsort (f.(fn_params) ++ f.(fn_vars))) e m1 ->
      reserve_boxes m1 (needed_stackspace f.(fn_id)) = Some (m2) ->
      bind_parameters ge e m2 f.(fn_params) vargs m2' ->
...
Inductive outcome_state_match
  ...
  | osm_return_some: forall v ty k',
      call_cont k' = call_cont k ->
      outcome_state_match e m f k
        (Out_return (Some (v, ty))) (ExprState f (Csyntax.Eval v ty) (Kreturn k') e m).
...
Lemma bigstep_to_steps:
  (forall e m a t m' v,
   eval_expression e m a t m' v ->
   forall f k,
   star (fun ge => step ge needed_stackspace) ge (ExprState f a k e m) t (ExprState f (Csyntax.Eval v (typeof a)) k e m'))
/\(forall e m K a t m' a',
   eval_expr e m K a t m' a' ->
   forall C f k, leftcontext K RV C ->
   simple a' = true /\ typeof a' = typeof a /\
   star (fun ge => step ge needed_stackspace) ge (ExprState f (C a) k e m) t (ExprState f (C a') k e m'))
/\(forall e m al t m' al',
   ...
   simplelist al' = true /\
   star (fun ge => step ge needed_stackspace) ge (ExprState f (C (Ecall a1 (exprlist_app al2 al) ty)) k e m)
              t (ExprState f (C (Ecall a1 (exprlist_app al2 al') ty)) k e m'))
/\(forall e m s t m' out,
   exec_stmt e m s t m' out ->
   forall f k,
   exists S,
   star (fun ge => step ge needed_stackspace) ge (State f s k e m) t S /\ outcome_state_match e m' f k out S)
/\(forall m fd args t m' res,
   eval_funcall m fd args t m' res ->
   forall k,
   is_call_cont k ->
   star (fun ge => step ge needed_stackspace) ge (Callstate fd args k m) t (Returnstate res k m')).
Proof.
  apply bigstep_induction; intros. 
- (* expression, general *)
  exploit (H0 (fun x => x) f k). constructor. intros [A [B C]].
  assert (match a' with Csyntax.Eval _ _ => False | _ => True end ->
          star (fun ge => step ge needed_stackspace) ge (ExprState f a k e m) t (ExprState f (Csyntax.Eval v (typeof a)) k e m')).
...
- (* valof *)
  exploit (H1 (fun x => C(Csyntax.Evalof x ty))).
    eapply leftcontext_compose; eauto. repeat constructor. intros [A [B D]].
...
- (* addrof *)
  ...
  simpl; intuition; eauto.
- (* unop *)
  exploit (H0 (fun x => C(Csyntax.Eunop op x ty))).
    eapply leftcontext_compose; eauto. repeat constructor. intros [A [B D]].
  simpl; intuition; eauto.
- (* binop *)
  exploit (H0 (fun x => C(Csyntax.Ebinop op x a2 ty))).
    eapply leftcontext_compose; eauto. repeat constructor. intros [A [B D]].
  exploit (H2 (fun x => C(Csyntax.Ebinop op a1' x ty))).
...
- (* seqand true *) 
  ...
  eapply star_left. left; eapply step_seqand_true; eauto.
    rewrite B; auto.
...
- (* seqor true *)
  exploit (H0 (fun x => C(Eseqor x a2 ty))).
    eapply leftcontext_compose; eauto. repeat constructor. intros [A [B D]].
  simpl; intuition. eapply star_right. eexact D. 
  left; eapply step_seqor_true; eauto.
  rewrite B; auto.
  traceEq. 
- (* condition *)
  ...
  eapply star_left. left; eapply step_condition; eauto.
  rewrite B; auto. instantiate (1:=b); auto.
...
- (* call internal *)
  ...
  + (* Out_normal *)
    assert (fn_return f = Tvoid /\ vres = Eval Vundef).
...
Lemma evalinf_funcall_steps:
  ...
  forever_N (fun ge => step ge needed_stackspace) lt ge O (Callstate fd args k m) t.
Proof.
  cofix COF.
  ...
  assert (COS: 
    ...
    forever_N (fun ge => step ge needed_stackspace) lt ge O (State f s k e m) t). 
  cofix COS.
  ...
  assert (COE:
    ...
    leftcontext K RV C ->
    forever_N (fun ge => step ge needed_stackspace) lt ge (esize a) (ExprState f (C a) k e m) t).
  cofix COE.
  ...
  assert (COEL:
    ...
    forever_N (fun ge => step ge needed_stackspace) lt ge (esizelist a)
...
+ (* valof *)
  eapply forever_N_star with (a2 := (esize a0)). apply star_refl. simpl; omega.
  eapply COE with (C := fun x => C(Csyntax.Evalof x ty)). eauto. 
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
+ (* deref *)
  eapply forever_N_star with (a2 := (esize a0)). apply star_refl. simpl; omega.
...
+ (* unop *)
  eapply forever_N_star with (a2 := (esize a0)). apply star_refl. simpl; omega.
  eapply COE with (C := fun x => C(Csyntax.Eunop op x ty)). eauto. 
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
+ (* binop left *)
  eapply forever_N_star with (a2 := (esize a1)). apply star_refl. simpl; omega.
  eapply COE with (C := fun x => C(Csyntax.Ebinop op x a2 ty)). eauto. 
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
+ (* binop right *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ H1 (fun x => C(Csyntax.Ebinop op x a2 ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_star with (a2 := (esize a2)). eexact R. simpl; omega.
  eapply COE with (C := fun x => C(Csyntax.Ebinop op a1' x ty)). eauto. 
...
+ (* call *)
  ...
  simpl. left; eapply step_call; eauto. congruence.
  reflexivity. 
...
+ (* if true/false *)
  ...
  right. eapply step_ifthenelse_2; eauto.  
  reflexivity. reflexivity.
  eapply COS; eauto. traceEq.
+ (* return some *)
  eapply forever_N_plus. apply plus_one; right; constructor.
...
Inductive bigstep_program_terminates (p: program) ns sg: trace -> int -> Prop :=
  | bigstep_program_terminates_intro: forall b f m0 m1 t r,
      ...
      eval_funcall ge ns m0 f nil t m1 (Eval (Vint r)) ->
...
Theorem bigstep_semantics_sound:
  forall p ns sg, bigstep_sound (bigstep_semantics p ns sg)
                           (semantics p ns sg).
Proof.
