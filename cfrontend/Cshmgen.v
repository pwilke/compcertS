...
Definition make_boolean (e: expr) (ty: type) :=
  ...
  | bool_default => Ebinop (Ocmp Cne) e (make_intconst Int.zero)   (**r should not happen *)
  end.
...
Definition transl_unop (op: CopSyntax.unary_operation) (a: expr) (ta: type) : res expr :=
  match op with
  | CopSyntax.Onotbool => make_notbool a ta
  | CopSyntax.Onotint => make_notint a ta
  | CopSyntax.Oneg => make_neg a ta
  | CopSyntax.Oabsfloat => make_absfloat a ta
  end.

Definition transl_binop (op: CopSyntax.binary_operation)
                        (a: expr) (ta: type)
                        (b: expr) (tb: type) : res expr :=
  match op with
  | CopSyntax.Oadd => make_add a ta b tb
  | CopSyntax.Osub => make_sub a ta b tb
  | CopSyntax.Omul => make_mul a ta b tb
  | CopSyntax.Odiv => make_div a ta b tb
  | CopSyntax.Omod => make_mod a ta b tb
  | CopSyntax.Oand => make_and a ta b tb
  | CopSyntax.Oor  => make_or a ta b tb
  | CopSyntax.Oxor => make_xor a ta b tb
  | CopSyntax.Oshl => make_shl a ta b tb
  | CopSyntax.Oshr => make_shr a ta b tb
  | CopSyntax.Oeq => make_cmp Ceq a ta b tb
  | CopSyntax.One => make_cmp Cne a ta b tb
  | CopSyntax.Olt => make_cmp Clt a ta b tb
  | CopSyntax.Ogt => make_cmp Cgt a ta b tb
  | CopSyntax.Ole => make_cmp Cle a ta b tb
  | CopSyntax.Oge => make_cmp Cge a ta b tb
  end.
...
Fixpoint transl_expr (a: ClightSyntax.expr) {struct a} : res expr :=
  match a with
  | ClightSyntax.Econst_int n _ =>
      OK(make_intconst n)
  | ClightSyntax.Econst_float n _ =>
      OK(make_floatconst n)
  | ClightSyntax.Econst_single n _ =>
      OK(make_singleconst n)
  | ClightSyntax.Econst_long n _ =>
      OK(make_longconst n)
  | ClightSyntax.Evar id ty =>
      make_load (Eaddrof id) ty
  | ClightSyntax.Etempvar id ty =>
      OK(Evar id)
  | ClightSyntax.Ederef b ty =>
      do tb <- transl_expr b;
      make_load tb ty
  | ClightSyntax.Eaddrof b _ =>
      transl_lvalue b
  | ClightSyntax.Eunop op b _ =>
      do tb <- transl_expr b;
      transl_unop op tb (typeof b)
  | ClightSyntax.Ebinop op b c _ =>
      do tb <- transl_expr b;
      do tc <- transl_expr c;
      transl_binop op tb (typeof b) tc (typeof c)
  | ClightSyntax.Ecast b ty =>
      do tb <- transl_expr b;
      make_cast (typeof b) ty tb
  | ClightSyntax.Efield b i ty => 
      match typeof b with
      | Tstruct _ fld _ =>
          do tb <- transl_expr b;
...
with transl_lvalue (a: ClightSyntax.expr) {struct a} : res expr :=
  match a with
  | ClightSyntax.Evar id _ =>
      OK (Eaddrof id)
  | ClightSyntax.Ederef b _ =>
      transl_expr b
  | ClightSyntax.Efield b i ty => 
      match typeof b with
      | Tstruct _ fld _ =>
...
Fixpoint transl_arglist (al: list ClightSyntax.expr) (tyl: typelist)
...
Fixpoint typlist_of_arglist (al: list ClightSyntax.expr) (tyl: typelist)
...
Fixpoint transl_statement (tyret: type) (nbrk ncnt: nat)
                          (s: ClightSyntax.statement) {struct s} : res stmt :=
  match s with
  | ClightSyntax.Sskip =>
      OK Sskip
  | ClightSyntax.Sassign b c =>
      do tb <- transl_lvalue b;
      do tc <- transl_expr c;
      do tc' <- make_cast (typeof c) (typeof b) tc;
      make_store tb (typeof b) tc'
  | ClightSyntax.Sset x b =>
      do tb <- transl_expr b;
      OK(Sset x tb)
  | ClightSyntax.Scall x b cl =>
  ...
  | ClightSyntax.Sbreak =>
      OK (Sexit nbrk)
  | ClightSyntax.Scontinue =>
      OK (Sexit ncnt)
  | ClightSyntax.Sreturn (Some e) =>
      do te <- transl_expr e;
      do te' <- make_cast (typeof e) tyret te;
      OK (Sreturn (Some te'))
  | ClightSyntax.Sreturn None =>
      OK (Sreturn None)
  | ClightSyntax.Sswitch a sl =>
      do ta <- transl_expr a;
  ...
  | ClightSyntax.Slabel lbl s =>
      do ts <- transl_statement tyret nbrk ncnt s;
      OK (Slabel lbl ts)
  | ClightSyntax.Sgoto lbl =>
      OK (Sgoto lbl)
  end

with transl_lbl_stmt (tyret: type) (nbrk ncnt: nat)
                     (sl: ClightSyntax.labeled_statements)
                     {struct sl}: res lbl_stmt :=
  match sl with
  | ClightSyntax.LSnil =>
      OK LSnil
  | ClightSyntax.LScons n s sl' =>
      do ts <- transl_statement tyret nbrk ncnt s;
