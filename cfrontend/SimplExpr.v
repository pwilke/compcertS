...
Function eval_simpl_expr (a: expr) : option expr_sym :=
  match a with
  | Econst_int n _ => Some(Eval (Vint n))
  | Econst_float n _ => Some(Eval (Vfloat n))
  | Econst_single n _ => Some(Eval (Vsingle n))
  | Econst_long n _ => Some(Eval (Vlong n))
  | Ecast b ty => 
      match eval_simpl_expr b with
      | None => None
      | Some v => sem_cast_expr v (typeof b) ty
      end
  | _ => None
  end.
...
Function makeif (a: expr) (s1 s2: statement) : statement :=
  ...
  (* | Some v => *)
  (*     match bool_expr v (typeof a) with *)
...
Definition transl_incrdecr (id: incr_or_decr) (a: expr) (ty: type) : expr :=
  match id with
  | Incr => ClightSyntax.Ebinop Oadd a (Econst_int Int.one type_int32s) (CopGeneric.incrdecr_type ty)
  | Decr => ClightSyntax.Ebinop Osub a (Econst_int Int.one type_int32s) (CopGeneric.incrdecr_type ty)
  end.
...
Fixpoint transl_expr (dst: destination) (a: Csyntax.expr) : mon (list statement * expr) :=
  ...
  | Csyntax.Eval (Eval (Vint n)) ty =>
      ret (finish dst nil (Econst_int n ty))
  | Csyntax.Eval (Eval (Vfloat n)) ty =>
      ret (finish dst nil (Econst_float n ty))
  | Csyntax.Eval (Eval (Vsingle n)) ty =>
      ret (finish dst nil (Econst_single n ty))
  | Csyntax.Eval (Eval (Vlong n)) ty =>
  ...
  | Csyntax.Eunop op r1 ty =>
      ...
      ret (finish dst sl1 (ClightSyntax.Eunop op a1 ty))
  | Csyntax.Ebinop op r1 r2 ty =>
      do (sl1, a1) <- transl_expr For_val r1;
      do (sl2, a2) <- transl_expr For_val r2;
      ret (finish dst (sl1 ++ sl2) (ClightSyntax.Ebinop op a1 a2 ty))
  | Csyntax.Ecast r1 ty =>
      do (sl1, a1) <- transl_expr For_val r1;
  ...
  | Csyntax.Eassignop op l1 r2 tyres ty =>
      ...
      | For_val | For_set _ =>
          ...
          ret (finish dst
                 (sl1 ++ sl2 ++ sl3 ++
                  Sset t (ClightSyntax.Ebinop op a3 a2 tyres) ::
                  make_assign a1 (Etempvar t tyres) :: nil)
                 (Ecast (Etempvar t tyres) ty1))
      | For_effects =>
          ret (sl1 ++ sl2 ++ sl3 ++ make_assign a1 (ClightSyntax.Ebinop op a3 a2 tyres) :: nil,
               dummy_expr)
      end
