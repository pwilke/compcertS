...
Definition empty_temp_env : temp_env := PTree.empty expr_sym.
...
Fixpoint create_undef_temps (temps: list ident) : temp_env :=
  ...
  | id :: temps' =>
    PTree.set id (Eval Vundef) (create_undef_temps temps')
 end.

(** Initialization of temporaries that are parameters. *)

Fixpoint bind_parameters (formals: list ident) (args: list expr_sym)
...
Definition eval_constant (cst: constant) : option expr_sym :=
  match cst with
...
Inductive alloc_variables: env -> mem ->
  ...
  | alloc_variables_cons:
      ...
      Mem.alloc m 0 (Z.max 0 sz) Normal  = Some (m1, b1) ->
      alloc_variables (PTree.set id (b1, (Z.max 0 sz)) e) m1 vars e2 m2 ->
      alloc_variables e m ((id, sz) :: vars) e2 m2.

Definition size_vars (l: list (ident * Z)) : Z :=
  List.fold_left
    (fun acc var => acc + align (Z.max 0 (snd var)) (two_power_nat MA)) l 0.
...
Inductive eval_expr: expr -> expr_sym -> Prop :=
  ...
  | eval_Eaddrof:
      forall id b,
        eval_var_addr e id b ->
        eval_expr (Eaddrof id) (Eval (Vptr b Int.zero))
...
Inductive step: state -> trace -> state -> Prop :=
  ...
  | step_internal_function: forall f vargs k m m1 e le m2,
      list_norepet (map fst f.(fn_vars)) ->
      list_norepet f.(fn_params) ->
      list_disjoint f.(fn_params) f.(fn_temps) ->
      alloc_variables empty_env m (VarSort.varsort' (fn_vars f)) e m1 ->
