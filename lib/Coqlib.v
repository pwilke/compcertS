...
Ltac destr_cond_match :=
  match goal with
      |- context[if ?B then _ else _] => destruct B eqn:?
    | |- context[match ?B with _ => _ end] => destruct B eqn:?
  end.
...
Ltac destr:= destr_no_dep.

Ltac des t :=
  destruct t eqn:?; try subst; simpl in *; intuition try congruence;
  repeat
    match goal with
    | H:_ = left _ |- _ => clear H
    | H:_ = right _ |- _ => clear H
...
Ltac destr_in H :=
  match type of H with
    context [match ?f with _ => _ end] => des f
  end.
...
Ltac simp H :=
  match type of H with
    | ?f _ = _ => unfold f in H
    | ?f _ _ = _ => unfold f in H
    | ?f _ _ _ = _ => unfold f in H
    | ?f _ _ _ _= _ => unfold f in H
    | ?f _ _ _ _ _ = _ => unfold f in H
    | ?f _ _ _ _ _ _ = _ => unfold f in H                                                   
  end;
  match type of H with
      context[match ?a with _ => _ end] => des a; inv H; auto
  end.
...
Proof.
  ...
  split. omega.
  generalize (Zmult_lt_reg_r _ _ _ H H1). intros. 
...
Lemma fold_left_map:
  forall {A B C} f (g: A -> C) (l: list A) (z: B),
    fold_left f (map g l) z =
    fold_left (fun acc elt => f acc (g elt)) l z.
Proof.
  induction l; simpl; intros; eauto.
Qed.

Lemma fold_left_ext:
  forall {A B} (f: A -> B -> A) g l z
    (ext: forall x y, f x y = g x y),
    fold_left f l z = fold_left g l z.
Proof.
  induction l; simpl; intros; eauto.
  rewrite ext.
  apply IHl. auto.
...
Lemma Forall_map:
  forall {A B} (f: A -> B) g P (l: list A),
    Forall P l ->
    (forall x, g (f x) = x) ->
    Forall (fun x => P (g x)) (map f l).
Proof.
...
Proof.
  unfold Plt, Ple.
  intros.
  rewrite <- Pos.compare_nlt_iff in H.
  rewrite <- Pos.compare_le_iff.
  rewrite Pos.compare_antisym.
  destruct (p ?= q)%positive ; simpl in * ; congruence.
Qed.
