...
(* for each fn_id -> a list of sizes of variables *)
let func_C_usage = (Hashtbl.create 17 : (P.t, int list) Hashtbl.t)
(* for each fn_id -> a string of a tikz drawing representing its activation record *)
let func_stacking = (Hashtbl.create 17 : (P.t, string) Hashtbl.t)
(* for each fn_id -> the name of the function *)
let func_names = (Hashtbl.create 17 : (P.t, string) Hashtbl.t)
(* for each fn_id -> extra, total stacksize *)
let func_extra_size = (Hashtbl.create 17 : (P.t, int*int) Hashtbl.t)
