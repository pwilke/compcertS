...
Definition sel_unop (op: Cminor.unary_operation) (arg: expr) : expr :=
  ...
  | Cminor.Ointuoffloat => intuoffloat hf arg
  | Cminor.Ofloatofint => floatofint arg
  | Cminor.Ofloatofintu => floatofintu hf arg
  | Cminor.Ointofsingle => intofsingle arg
  | Cminor.Ointuofsingle => intuofsingle hf arg
  | Cminor.Osingleofint => singleofint arg
  | Cminor.Osingleofintu => singleofintu hf arg
...
Fixpoint sel_stmt (ge: Cminor.genv) (s: Cminor.stmt) : res stmt :=
  ...
  | Cminor.Sstore chunk addr rhs =>
    OK (store chunk (sel_expr addr) (sel_expr rhs))
...
(*   intros. *)
(*   etransitivity; eauto. *)
(*   apply align_le; auto. omega. *)
(* Qed. *)

(* Lemma epos : (8 > 0). Proof. omega. Qed. *)
