...
Lemma find_function_translated:
  ...
  find_function tge m ros rs = Some (transf_fundef fd).
Proof.
...
Variable needed_stackspace : AST.ident -> nat .

Lemma step_simulation:
  forall S1 t S2, RTL.step ge needed_stackspace  S1 t S2 ->
  forall S1', match_states S1 S1' ->
  exists S2', RTL.step tge needed_stackspace  S1' t S2' /\ match_states S2 S2'.
Proof.
...
- (* load *)
  econstructor; split.
  assert (eval_addressing tge (Eval (Vptr sp Integers.Int.zero)) addr rs##args = Some a).
...
- (* cond *)
  ...
  replace (if negb (Integers.Int.eq vb Integers.Int.zero) then renum_pc (pnum f) ifso else renum_pc (pnum f) ifnot)
     with (renum_pc (pnum f) (if negb (Integers.Int.eq vb Integers.Int.zero) then ifso else ifnot)).
  constructor; auto. eapply reach_succ; eauto. simpl.
  destruct (Integers.Int.eq); auto.
  destruct (Integers.Int.eq); auto. 
- (* jumptbl *)
  econstructor; split.
...
Lemma transf_initial_states:
  forall sg S1, RTL.initial_state prog sg S1 ->
  exists S2, RTL.initial_state tprog sg S2 /\ match_states S1 S2.
Proof.
