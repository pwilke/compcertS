...
Lemma functions_translated:
  ...
  Genv.find_funct m tge v = Some (tunnel_fundef f).
...
Lemma find_function_translated:
  ...
  find_function tge m ros ls = Some (tunnel_fundef f).
Proof.
...
Lemma tunnel_step_correct:
  forall st1 t st2, step ge needed_stackspace  st1 t st2 ->
  forall st1' (MS: match_states st1 st1'),
  (exists st2', step tge needed_stackspace  st1' t st2' /\ match_states st2 st2')
...
Proof.
  ...
  assert (DEFAULT: branch_target f pc = pc ->
    (exists st2' : state,
     step tge needed_stackspace  (State ts (tunnel_function f) sp (branch_target f pc) rs m) E0 st2'
  ...
  - (* Lcond *)
    ...
    destruct (Integers.Int.eq i Integers.Int.zero); econstructor; eauto.
  - (* Ljumptable *)
    left; simpl; econstructor; split.
    eapply exec_Ljumptable; eauto. 
    rewrite list_nth_z_map. change U.elt with node. rewrite H0. reflexivity.
    econstructor; eauto. 
  - (* Lreturn *)
    left; simpl; econstructor; split.
...
Proof.
  ...
  - econstructor; eauto.
    + eapply Genv.init_mem_transf; eauto.
      unfold fid; des a; des b0. inv H5.
      auto.
...
Proof.
  intros; eapply forward_simulation_opt.
