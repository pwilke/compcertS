...
Definition sel_val (k: equation_kind) (v: expr_sym) : expr_sym :=
  match k with
  | Full => v
  | Low => Val.loword v
  | High => Val.hiword v
...
Lemma satisf_undef_reg:
  forall rs ls e r,
  satisf rs ls e ->
  satisf (rs#r <- (Eval Vundef)) ls e.
Proof.
  intros; red; intros. rewrite Regmap.gsspec. destruct (peq (ereg q) r); auto.
  red; intros; simpl; destruct (ekind q); simpl; auto.
Qed.
...
Lemma add_equations_args_lessdef:
  ...
  add_equations_args rl tyl ll e = Some e' ->
  satisf rs ls e' ->
  list_forall2 wt (rs##rl) tyl ->
  Val.lessdef_list (rs##rl) (decode_longs tyl (map ls ll)).
Proof.
  intros until e'.
  functional induction (add_equations_args rl tyl ll e); simpl; intros;
                   try solve [constructor]; myinv;
                   try (constructor; auto; [idtac]).
  - destruct H5 as [A|A]; dex; destr; [|red; intros; rewrite nwt_expr_undef; auto].
    des t'.
    generalize (val_longofwords_eq (rs#r1) H1).
    intros A.
    red; intros. rewrite <- A; auto. apply Val.longofwords_lessdef. 
...
Proof.
  ...
  - inversion A. rewrite H3; rewrite H4. rewrite Regmap.gss.
    ...
    + simpl.
      red; intros; simpl.
      destruct (eSexpr alloc em rs#src1); simpl; auto.
      destr; auto. 
      rewrite Int64.hi_ofwords. auto.
    + red; intros. exploit H0. eexact D. simpl. eauto. 
...
Proof.
  ...
  red; intros.
  exploit H0. eexact B. simpl. eauto. 
  rewrite Regmap.gso. apply H0; auto. eapply reg_unconstrained_sound; eauto.
Qed.
...
Proof.
  ...
  inversion A. rewrite H3; rewrite H4. simpl. rewrite Regmap.gss. 
  red; intros. exploit H0. eexact B. simpl. eauto. 
  rewrite Regmap.gso. apply H0; auto. eapply reg_unconstrained_sound; eauto.
Qed.
...
Lemma loc_type_compat_charact:
  ...
  EqSet.In q e ->
  (sel_type (ekind q) (env (ereg q))) = (Loc.type l) \/ Loc.diff l (eloc q).
Proof.
  ...
  - left. exploit H; eauto.  apply eqs_same; auto.
    des (sel_type (ekind q) (env (ereg q))); des (Loc.type l); discriminate.
...
Lemma well_typed_move_charact:
  ...
  match l with
  | R mr => True
  | S sl ofs ty => wt (sel_val k rs#r) ty
  end.
Proof.
  unfold well_typed_move; intros.
  destruct l as [mr | sl ofs ty]; auto.
  exploit loc_type_compat_charact; eauto. intros [A | A].
  - simpl in A.
    generalize (H1 r).
    destruct k; simpl in A; intros.
    + des (env r).
    + subst; simpl.
      destruct (wt_expr_dec (rs # r) Tlong).
      left; exists AST.Tint; simpl; auto.
      right. intros [t A ]. destr. 
    + subst; simpl.
      destruct (wt_expr_dec (rs # r) Tlong).
...
(*     + apply (H2 _ B).  *)
(*     + red; intros. *)
(*       destruct (Val.eq_val_dec (Vundef) (eSexpr alloc em (sel_val k rs#r))) as [A|A]. *)
(*       rewrite <- A. auto. *)
(*       eapply Values.Val.lessdef_trans. *)
(*       apply (H2 _ B). simpl. *)
(*       generalize (H2 _ B alloc em). simpl. *)
(*       intro C; inv C; destr. *)
(*       rewrite H7 in *. *)
(*       destruct H4 as [[t [C D]]|C]; auto. *)
(*       *  *)
...
Qed.
  ...
  Proof.
    ...
    inv H0; auto. simpl.
    destruct H as [[t [A B]]|NWT] .
    - generalize (expr_type _ _ A alloc em).
      generalize (expr_type _ _ A alloc em). rewrite H3 at 1.
      intros C D. generalize (expr_type' _ _ _ _ C).
      des t; des ty; try rewrite <- H3; seval; auto; repeat (revert Heqv1; destr).
...
- (* no params *)
  simpl. red; intros. rewrite Regmap.gi.
  red; intros; destruct (ekind q); simpl; auto.
- (* a param of type Tlong *)
  ...
  apply Val.hiword_lessdef; auto. apply Val.val_hiword_longofwords.
  apply Val.lessdef_trans with (Val.loword (Val.longofwords (ls l1) (ls l2))).
  apply Val.loword_lessdef; auto. apply Val.val_loword_longofwords.
  eapply IHb; eauto.
...
Lemma find_function_tailcall:
  forall tge m ros ls1 ls2,
  ros_compatible_tailcall ros = true ->
  find_function tge m ros (return_regs ls1 ls2) = find_function tge m ros ls2.
Proof.
...
Lemma find_function_translated:
  forall m ros rs fd ros' e e' ls,
  RTL.find_function ge m ros rs = Some fd ->
  add_equation_ros ros ros' e = Some e' ->
  satisf rs ls e' ->
  exists tfd,
  LTL.find_function tge m ros' ls = Some tfd /\ transf_fundef fd = OK tfd.
Proof.
  unfold RTL.find_function, LTL.find_function; intros.
  destruct ros as [r|id]; destruct ros' as [r'|id']; simpl in H0; MonadInv.
  - (* two regs *)
    generalize (add_equation_lessdef _ _ _ _ H1).
    intros LD. 
    eapply functions_translated; eauto. simpl in *.
    exploit Genv.find_funct_lessdef; eauto.
    reflexivity.
  - (* two symbols *)
...
Lemma exec_moves:
  ...
  wt_regset env rs ->
  exists ls',
    star (fun ge => step ge needed_stackspace ) tge (Block s f sp (expand_moves mv bb) ls m)
  ...
  | match_stackframes_cons:
      forall res f sp pc rs s tf bb ls ts sg an e env
        ...
        (STEPS: forall v ls1 m,
           ...
           exists ls2,
           star (fun ge => LTL.step ge needed_stackspace ) tge (Block ts tf (Eval (Vptr sp Int.zero)) bb ls1 m)
                          E0 (State ts tf (Eval (Vptr sp Int.zero)) pc ls2 m)
           /\ satisf (rs#res <- v) ls2 e),
      match_stackframes
        (RTL.Stackframe res f sp pc rs :: s)
        (LTL.Stackframe tf (Eval (Vptr sp Int.zero)) ls bb :: ts)
        sg.
...
Inductive match_states: RTL.state -> LTL.state -> Prop :=
  | match_states_intro:
      ...
      match_states (RTL.State s f sp pc rs m)
                   (LTL.State ts tf (Eval (Vptr sp Int.zero)) pc ls m')
  | match_states_call:
      forall s f args m ts tf ls m'
        ...
        (MEM: mem_lessdef m m')
        (WTARGS: list_forall2 wt args (sig_args (funsig tf))),
...
Proof.
  intros. 
  assert (forall ty, In ty (type_of_addressing addr) -> ty = AST.Tint).
  { intros. destruct addr; simpl in H1; intuition. }
  inv H.
  assert (env r = AST.Tint).
...
Lemma step_simulation:
  forall S1 t S2, RTL.step ge needed_stackspace  S1 t S2 -> wt_state S1 ->
  forall S1', match_states S1 S1' ->
  exists S2', plus (fun ge => LTL.step ge needed_stackspace ) tge S1' t S2' /\ match_states S2 S2'.
Proof.
...
(* op move *)
- generalize (wt_exec_Iop _ _ _ (Eval (Vptr sp Int.zero))
              _ _ _ _ _ _ I WTI H0 WTRS). 
  intros WTRS'.
  simpl in H0. inv H0.  
...
(* op makelong *)
- generalize (wt_exec_Iop _ _ _ (Eval (Vptr sp Int.zero))
              _ _ _ _ _ _ I WTI H0 WTRS). intros WTRS'.
  simpl in H0. inv H0. 
  exploit (exec_moves mv); eauto. intros [ls1 [X Y]]. 
  econstructor; split.
  eapply plus_left. econstructor; eauto. 
...
- generalize (wt_exec_Iop _ _ _ (Eval (Vptr sp Int.zero))
  ...
  exploit eval_operation_rel; eauto. eapply Val.lessdef_list_inv. eauto.
  intros [v' [F G]]. 
...
- generalize (wt_exec_Iload _ _ _ _ _ _ _ _ _ _ _ WTI H1 WTRS). intros WTRS'.
  exploit (exec_moves mv1); eauto. intros [ls1 [A1 B1]]. 
  exploit transfer_use_def_satisf; eauto. intros [X Y].
  exploit eval_addressing_rel; eauto. eapply Val.lessdef_list_inv; eauto.
  intros [a' [F G]].
  exploit loadv_addr_rel; eauto. intros [v' [P Q]]. 
  exploit (exec_moves mv2); eauto. intros [ls2 [A2 B2]].
  econstructor; split.
  eapply plus_left. econstructor; eauto. 
  eapply star_trans. eexact A1. 
...
(* load pair *)
- generalize (wt_exec_Iload _ _ _ _ _ _ _ _ _ _ _ WTI H1 WTRS). intros WTRS'.
  exploit Mem.loadv_int64_split; eauto. intros (v1 & v2 & LOAD1 & LOAD2 & V1 & V2).
  ...
  exploit eval_addressing_rel. apply wf_mr_ld. eapply Val.lessdef_list_inv. eexact LD1. eauto.
  intros [a1' [F1 G1]].
  exploit loadv_addr_rel. apply wf_mr_ld. apply wf_mr_norm_ld. eauto. eexact LOAD1.
  eexact G1. intros (v1'' & LOAD1' & LD2).
  ...
  exploit eval_offset_addressing. eauto. eexact H0.
  intros [e4 [EA SE]].
  exploit eval_addressing_rel. apply wf_mr_ld. eapply Val.lessdef_list_inv. eexact LD3. eexact EA.
  intros [a2' [F2 G2']].
  assert (G2: Val.lessdef (Val.add a (Eval (Vint (Int.repr 4)))) a2').
  {
    red; intros. rewrite <- SE. apply G2'.
  } clear G2'.
  exploit loadv_addr_rel. apply wf_mr_ld. apply wf_mr_norm_ld. eauto.
  eexact LOAD2. eexact G2.
  intros (v2'' & LOAD2' & LD4).
...
- generalize (wt_exec_Iload _ _ _ _ _ _ _ _ _ _ _ WTI H1 WTRS). intros WTRS'.
  ...
  { eapply add_equations_lessdef; eauto. }
  exploit eval_addressing_rel. apply wf_mr_ld. apply Val.lessdef_list_inv. eexact LD1.
  eauto. intros [a1' [F1 G1]].
  exploit loadv_addr_rel. apply wf_mr_ld. apply wf_mr_norm_ld. eauto. eexact LOAD1. eexact G1. intros (v1'' & LOAD1' & LD2).
...
- generalize (wt_exec_Iload _ _ _ _ _ _ _ _ _ _ _ WTI H1 WTRS). intros WTRS'.
  ...
  assert (LD1: Val.lessdef_list rs##args (reglist ls1 args')).
  { eapply add_equations_lessdef; eauto. }
  exploit eval_offset_addressing; eauto.
  intros [e2 [EA SE]].
  exploit eval_addressing_rel. apply wf_mr_ld. apply Val.lessdef_list_inv. eexact LD1. eexact EA.
  intros [a1' [F1 G1]].
  exploit loadv_addr_rel. apply wf_mr_ld. apply wf_mr_norm_ld. eauto. eexact LOAD2.
  red; intros. rewrite <- SE. apply G1. intros (v2'' & LOAD2' & LD2).
...
- exploit exec_moves; eauto. intros [ls1 [X Y]].
  ...
  exploit eval_addressing_rel; eauto. apply Val.lessdef_list_inv; eauto.
  intros [a' [F G]].
  exploit storev_rel; eauto. intros [m'' [P Q]].
  econstructor; split.
  eapply plus_left. econstructor; eauto. 
...
(* store 2 *)
- exploit storev_int64_split; eauto. 
  ...
  assert (SAT2: satisf rs ls2 e1).
    ...
    exploit eval_addressing_rel. apply wf_mr_ld. apply Val.lessdef_list_inv. eexact LD1.
    eauto. intros [a1' [F1 G1]].
  assert (F1': eval_addressing tge (Eval (Vptr sp Int.zero)) addr (reglist ls1 args1') = Some a1').
    rewrite <- F1. apply eval_addressing_preserved. exact symbols_preserved.
  exploit storev_rel. apply wf_mr_ld. apply wf_mr_norm_ld. eauto. eexact STORE1.
  eexact G1. eauto.
  ...
  assert (LD4: Val.lessdef (sel_val kind_second_word rs # src) (ls3 (R src2'))).
  red; intros.
  exploit add_equation_lessdef. eapply add_equations_satisf. eexact Heqo. eexact V.
  simpl. eauto. 
  exploit eval_addressing_rel. apply wf_mr_ld. apply Val.lessdef_list_inv. eexact LD3.
  eauto. intros [a2' [F2 G2]].
  assert (F2': eval_addressing tge (Eval (Vptr sp Int.zero)) addr (reglist ls3 args2') = Some a2').
    rewrite <- F2. apply eval_addressing_preserved. exact symbols_preserved.
  exploit eval_offset_addressing. eauto. eexact F2'. intros [e3 [EA SE]].
  exploit storev_rel. apply wf_mr_ld. apply wf_mr_norm_ld. 
  eexact EXT1. eexact STORE2.
  ...
  eapply star_two.
  econstructor. rewrite EA. eauto. eexact STORE2'. eauto. 
...
- set (sg := RTL.funsig fd) in *.
  set (args' := loc_arguments sg) in *.
  exploit free_mem_rel; eauto. intros [mm' [P Q]].
  exploit MemReserve.release_boxes_rel; eauto. intros [m2' [R S]].
  ...
  replace (fn_id tf) with (RTL.fn_id f); eauto.
  destruct (transf_function_inv _ _ FUN); auto.
  traceEq. traceEq.
  econstructor; eauto.
...
- assert (WTRS': wt_regset env (rs#res <- v)) by (eapply wt_exec_Ibuiltin; eauto).
  exploit (exec_moves mv1); eauto. intros [ls1 [A1 B1]]. 
  exploit external_call_rel. apply wf_mr_ld. apply wf_mr_norm_ld. eauto.
  apply Val.lessdef_list_inv.
  eapply add_equations_args_lessdef; eauto.
  inv WTI. rewrite <- H4. apply wt_regset_list; auto. eexact H0.
  intros [v' [m'' [F [G J]]]].
...
(* (* annot *) *)
(* - exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].   *)
(*   exploit (external_call_ld). eauto. eapply add_equations_args_lessdef; eauto. *)
...
(* cond *)
- exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  exploit eval_condition_rel. apply wf_mr_ld. apply Val.lessdef_list_inv.
  eapply add_equations_lessdef; eauto. eauto. eauto.
  intros [b1 [A B]].
  econstructor; split.
  eapply plus_left. econstructor; eauto. 
  eapply star_right. eexact A1.
  econstructor. eauto.
  generalize (rel_norm _ wf_mr_ld wf_mr_norm_ld _ _ _ _ MEM B). rewrite H1.
  rewrite lessdef_val.
  intro C; inv C. eauto. eauto.
  eauto. eauto. traceEq. 
  exploit satisf_successors; eauto.
  instantiate (1 := if negb (Int.eq vb Int.zero) then ifso else ifnot). simpl. destr. 
  eapply can_undef_satisf. eauto. eapply add_equations_satisf; eauto.
  intros [enext [U V]]. 
  econstructor; eauto.
...
(* jumptable *)
- exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  assert (Val.lessdef (rs#arg) (ls1 (R arg'))).
  {
    apply (add_equation_lessdef _ _ _ _ B1).
  }
  econstructor; split.
  eapply plus_left. econstructor; eauto. 
  eapply star_right. eexact A1.
  econstructor.
  generalize (rel_norm _ wf_mr_ld wf_mr_norm_ld _ _ _ _ MEM H2).
  rewrite lessdef_val.
  rewrite H0.  intro A; inv A. eauto.
  eauto. eauto. eauto. traceEq. 
  exploit satisf_successors; eauto.
...
- destruct (transf_function_inv _ _ FUN). 
  exploit free_mem_rel; eauto. rewrite H11. intros [mm' [P Q]].
  exploit MemReserve.release_boxes_rel; eauto. rewrite H13. intros [mm'' [R S]].
  inv WTI; MonadInv.
+ (* without an argument *)
  exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  econstructor; split.
  eapply plus_left. econstructor; eauto. 
  eapply star_right. eexact A1.
  econstructor. eauto. eauto. traceEq. traceEq.
  econstructor; eauto. 
  ...
  apply return_regs_agree_callee_save.
  red; simpl; auto. 
  left; eexists; split; [|apply subtype_refl]; auto.
+ (* with an argument *)
  exploit (exec_moves mv); eauto. intros [ls1 [A1 B1]].
  econstructor; split. 
  eapply plus_left. econstructor; eauto. 
  ...
  replace (map (return_regs (parent_locset ts) ls1) (map Locations.R (loc_result (RTL.fn_sig f))))
     with (map ls1 (map Locations.R (loc_result (RTL.fn_sig f)))).
  ...
  apply return_regs_agree_callee_save.
  unfold proj_sig_res. rewrite <- H12; rewrite H15. apply WTRS. 

(* internal function *)
- monadInv FUN. simpl in *.
  destruct (transf_function_inv _ _ EQ). 
  destruct (alloc_mem_rel _ wf_mr_ld _ _ _ _ _ _ H _ MEM) as [m2' [A B]].
  edestruct (MemReserve.reserve_boxes_rel) as [m3 [C D]]; eauto.
...
- inv STACKS.
  ...
  econstructor; eauto.
  apply wt_regset_assign2; auto. rewrite WTRES0; auto.
Qed.

Lemma initial_states_simulation:
  forall sg st1, RTL.initial_state prog sg st1 ->
  exists st2, LTL.initial_state tprog sg st2 /\ match_states st1 st2.
Proof.
  intros. inv H.
  ...
  - econstructor; eauto.
    + eapply Genv.init_mem_transf_partial; eauto.
      ...
      unfold transf_function in EQ. repeat destr_in EQ. monadInv EQ.
       unfold check_function in EQ0. repeat destr_in EQ0. 
...
Proof.
  ...
  generalize (rel_norm _ wf_mr_ld wf_mr_norm_ld _ _ _ _ MEM LD). rewrite lessdef_val. rewrite H1; intro A; inv A; auto. 
Qed.
...
Theorem transf_program_correct:
  forall sg, 
    forward_simulation
      ...
      (LTL.semantics tprog needed_stackspace sg ).
Proof.
  intros.
  set (ms := fun s s' => wt_state s /\ match_states s s').
  eapply forward_simulation_plus with (match_states := ms). 
- exact symbols_preserved.
- intros. unfold RTL.semantics in *; simpl in *.
...
- intros. destruct H0.
  unfold RTL.semantics in *; simpl in *.
  exploit step_simulation; eauto. intros [s2' [A B]].
  exists s2'; split. exact A. split.
  eapply subject_reduction; eauto. eexact wt_prog. auto. 
Qed.
