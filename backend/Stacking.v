...
Lemma size_pos: forall f, 0 <= fe_size (make_env (function_bounds f)).
Proof.
  intros.
  set (b := function_bounds f).
...
Definition transf_function (f: Linear.function) : res Mach.function :=
  ...
  if negb (wt_function f) then
    Error (msg "Ill-formed Linear code")
       ...
       else (* if zeq (needed_stackspace (Linear.fn_id f)) fe.(fe_size) then *)
              OK (Mach.mkfunction
                    (Linear.fn_id f)
                    (Linear.fn_sig f)
                    (transl_body f fe)
                    ...
                    (Zle_trans _ _ _ (size_pos f) (align_le _ 8 epos))
                    (Z.to_nat (align (fe_size fe) 8 - Linear.fn_stacksize f))

                 )
            (* else Error (msg "The oracle-given size was not exact.") *).
