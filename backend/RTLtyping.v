...
Definition type_instr (e: S.typenv) (i: instruction) : res S.typenv :=
  ...
  | Iload chunk addr args dst s =>
      ...
      S.set e1 dst (AST.type_of_chunk chunk)
  | Istore chunk addr args src s =>
      do x <- check_successor s;
      do e1 <- S.set_list e args (type_of_addressing addr);
      S.set e1 src (AST.type_of_chunk chunk)
...
Lemma type_ros_sound:
  ...
  match ros with inl r => te r = AST.Tint | inr s => True end.
Proof.
...
Lemma wt_regset_list:
  ...
  wt_regset env rs ->
  forall rl,
    list_forall2 wt rs##rl (List.map env rl).
Proof.
...
Lemma wt_init_regs:
  forall env rl args,
    list_forall2 wt args (List.map env rl) ->
    wt_regset env (init_regs args rl).
Proof.
  induction rl; simpl; intros.
  - red; intros. rewrite Regmap.gi. left; eexists; destr; apply subtype_refl. 
  - destruct args.
    red; intros. rewrite Regmap.gi.
    left; eexists; destr; apply subtype_refl. 
    inv H; apply wt_regset_assign2; auto.
Qed.

Require Import Classical_Prop.

Lemma eval_addressing_nt:
  forall F V (ge: Genv.t F V) sp a rs args v
         (EOP : eval_addressing ge sp a rs ## args = Some v)
         (A : (exists tl : list typ, list_forall2 wt rs ## args tl) -> False),
    ~ exists t, wt_expr v t.
Proof.
  red; intros.
  dex; myinv; simpl in *; intuition;
  apply A.
  des a; myinv; simpl in *; intuition;
  first 
    [solve [
         exists (AST.Tint::nil); constructor; auto; left; eexists; destr; try apply subtype_refl; destr
       ] |
     solve [
         exists (AST.Tint:: AST.Tint::nil); repeat (apply list_forall2_cons || apply list_forall2_nil); auto; left; eexists; destr; try apply subtype_refl; destr
       ] |
     solve [exists nil; constructor; auto] |
     idtac].
  des (Genv.symbol_address' ge i i0). inv EOP. 
  exists (AST.Tint::nil); constructor; auto; left; eexists; destr; try apply subtype_refl; destr.
  des (Genv.symbol_address' ge i0 i1). inv EOP. 
  exists (AST.Tint::nil); constructor; auto; left; eexists; destr; try apply subtype_refl; destr.
Qed.

Lemma eval_condition_nt:
  forall c l v
         (EOP : eval_condition c l = Some v)
         (A : (exists tl : list typ, list_forall2 wt l tl) -> False),
    ~ exists t, wt_expr v t.
Proof.
  red; intros; dex.
  des c; myinv; destr; apply A; eexists; repeat econstructor; try apply subtype_refl;
  match goal with
    H: wt_expr ?e ?t |- wt_expr ?e _ => instantiate (1:= Normalise.typ_of_styp t); destr
  end.
Qed.

Lemma all_wt:
  forall e, exists t, wt e t.
Proof.
  intros.
  destruct (classic (exists t, wt_expr e t)).
  dex; exists (Normalise.typ_of_styp t); destr; left; eexists; split; try apply subtype_refl.
  des t.
  exists AST.Tint; right; auto.
Qed.

(* Lemma eval_operation_nt: *)
(*   forall F V (ge : Genv.t F V) sp op rs args m v  *)
(*          (EOP : eval_operation ge sp op rs ## args m = Some v) *)
(*          (A : ~ (exists tl : list typ, list_forall2 wt rs ## args tl)), *)
(*     wt v (snd (type_of_operation op)). *)
(* Proof. *)
(*   intros. *)
(*   generalize isTint isTlong isTfloat isTsingle . intros.  *)
(*   des op; *)
...
(*   right; intro; dex; intuition. apply A; exists (t::nil); destr. *)
(*   right. eapply eval_addressing_nt; eauto. *)
(*   right. eapply eval_condition_nt; eauto. *)
(* Qed. *)
...
Proof.
  red; intros.
  des a; myinv; simpl in *; intuition;
  unfold Genv.symbol_address' in *; try (revert EOP; destr; inv EOP); destr;
  try solve [des (wt_expr_dec e Tint); try des (wt_expr_dec e0 Tint); right; intro; dex; intuition].
...
Lemma eval_operation_wt:
  forall F V (ge : Genv.t F V) sp op rs args v 
         (EOP : eval_operation ge sp op rs ## args = Some v)
         (diff: op <> Omove),
...
Proof.
  ...
  try solve [right; intro; dex; intuition].
  unfold Genv.symbol_address' in EOP; revert EOP; destr; inv EOP.
  simpl; auto.
...
Lemma wt_exec_Iop:
  forall (ge: genv) env f sp op args res s rs v,
...
Proof.
  intros ge env f sp op args res s rs v WTsp  WTi EOP WTrs. inv WTi. 
  - apply wt_regset_assign2; auto.
    rewrite H1.
    simpl in EOP. inv EOP. auto.
  - eapply wt_regset_assign2; auto.
    rewrite H5.
    apply weak_wt_wt'.
    eapply eval_operation_wt; eauto.
Qed.
...
Proof.
  intros env f chunk addr args dst s m a v rs WTi ML WTrs.
  unfold Mem.loadv in ML.
  revert ML; destr_cond_match; try discriminate. inv WTi.
  intros.
  eapply wt_regset_assign2; eauto. rewrite H5.
  destruct (classic (exists t, Val.has_type v t)); auto.
  dex; eapply Mem.load_type in ML; eauto. right; auto. 
Qed.
...
Inductive wt_state: state -> Prop :=
  ...
  | wt_state_call:
      ...
      wt_fundef f ->
      list_forall2 wt args (sig_args (funsig f)) ->
...
Proof.
  ...
  - econstructor; eauto. rewrite H3.
    unfold proj_sig_res. rewrite H. auto. 
Qed.
...
Lemma subject_reduction:
  forall st1 t st2,
    step ge needed_stackspace  st1 t st2 ->
    forall (WT: wt_state st1),
      wt_state st2.
Proof.
  ...
  - (* Icall *)
    assert (wt_fundef fd).
    {
      ...
      - pattern fd. eapply Genv.find_funct_prop; eauto.
    ...
    inv WTI. rewrite <- H8. eapply wt_regset_list; eauto.
  - (* Itailcall *)
    assert (wt_fundef fd).
    {
      destruct ros; simpl in H0. 
      - 
        pattern fd. eapply Genv.find_funct_prop ; eauto.
      - revert H0; destr_cond_match; try discriminate. intro.
        pattern fd. apply Genv.find_funct_ptr_prop with fundef unit p b.
        exact wt_p. exact H0.
    }
    econstructor; eauto.
  ...
  - (* Ireturn *)
    econstructor; eauto. 
    inv WTI; simpl.
    + left; eexists; split; try apply subtype_refl; destr. 
    + simpl in *. unfold proj_sig_res; rewrite H3.
      apply (WT_RS arg). 
  - (* internal function *)
    simpl in *. inv H6.
    econstructor; eauto.
  ...
  - (* return *)
    ...
    apply wt_regset_assign2; auto. rewrite H10; auto. 
Qed.

Lemma wt_initial_state:
  forall sg S, initial_state p sg S -> wt_state S.
Proof.
