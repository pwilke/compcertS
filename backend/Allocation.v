...
Definition loc_type_compat (env: regenv) (l: loc) (e: eqs) : bool :=
  EqSet2.for_all_between
    (fun q => typ_eq (sel_type (ekind q) (env (ereg q))) (Loc.type l))
...
Definition check_entrypoints_aux (rtl: RTL.function) (ltl: LTL.function)
  ...
  assertion (peq (RTL.fn_id rtl) (LTL.fn_id ltl));
  Some tt.
...
Definition check_function (rtl: RTL.function) (ltl: LTL.function) (env: regenv): res unit :=
  ...
  | Some a => if ident_eq (RTL.fn_id rtl) (LTL.fn_id ltl)
             then check_entrypoints rtl ltl env bsh a
             else Error (msg "function identifiers should not be altered by register allocation")
  end.
