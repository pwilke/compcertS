...
Lemma find_function_translated:
  forall m ros ls f,
  LTL.find_function ge m ros ls = Some f ->
  exists tf,
  find_function tge m ros ls = Some tf /\ transf_fundef f = OK tf.
Proof.
...
Lemma starts_with_correct:
  ...
  find_label lbl c2 = Some c3 ->
  plus (fun ge => step ge needed_stackspace ) tge (State s f sp c1 ls m)
...
Lemma add_branch_correct:
  ...
  plus (fun ge => step ge needed_stackspace ) tge
       (State s tf sp (add_branch lbl k) ls m)
...
Inductive match_states: LTL.state -> Linear.state -> Prop :=
  ...
  | match_states_cond_taken:
      forall s f sp pc ls m tf ts cond args c bcond
        (STACKS: list_forall2 match_stackframes s ts)
        (TRF: transf_function f = OK tf)
        (REACH: (reachable f)!!pc = true)
        (JUMP: eval_condition cond (reglist ls args) = Some bcond)
        (NORM: Mem.mem_norm m bcond = Vtrue),
  ...
  | match_states_jumptable:
      forall s f sp pc ls m tf ts arg tbl c n
        ...
        (TRF: transf_function f = OK tf)
        (REACH: (reachable f)!!pc = true)
        (ARG: Mem.mem_norm m (ls (R arg)) = Vint n)
...
Theorem transf_step_correct:
  forall s1 t s2, LTL.step ge needed_stackspace  s1 t s2 ->
                  forall s1' (MS: match_states s1 s1'),
                    (exists s2', plus (fun ge => Linear.step ge needed_stackspace ) tge s1' t s2' /\ match_states s2 s2')
...
Proof.
  ...
  -(* Lcond *)
    ...
    +(* branch if cond is false *)
      ...
      generalize (eval_negate_condition cond (reglist rs args)).
      rewrite H; simpl. 
      destruct (eval_condition (negate_condition cond) (reglist rs args)) eqn:?; intros; try (exfalso; auto; fail).
      destruct (negb (Int.eq i Int.zero)) eqn:?.
      * (* cond is true: no branch *)
        left; econstructor; split.
        apply plus_one. eapply exec_Lcond_false; eauto.
        erewrite Mem.same_eval_eqm ; eauto.
        erewrite Mem.normalise_int_notbool; eauto.
        unfold Int.notbool.
        destruct (Int.eq i Int.zero); simpl in *; intuition try congruence.
    ...
    + (* branch if cond is true *)
      ...
      * (* cond is true: branch is taken *)
        ...
        setoid_rewrite H0.
        exploit (eval_cond_norm_0_1); eauto. intuition; subst; auto.
        rewrite Int.eq_true in Heqb0.
        discriminate.
      * (* cond is false: no branch *)
        left; econstructor; split.
        apply plus_one. eapply exec_Lcond_false; eauto. 
        setoid_rewrite H0. exploit eval_cond_norm_0_1; eauto. intuition; subst; auto.
        rewrite Int.eq_false in Heqb0. discriminate. discriminate.
        econstructor; eauto.
  - (* Ljumptable *)
  ...
  - (* external function *)
    monadInv H7. left; econstructor; split.
...
Lemma transf_initial_states:
  forall sg st1, LTL.initial_state prog sg st1 ->
  exists st2, Linear.initial_state tprog sg st2 /\ match_states st1 st2.
Proof.
  intros. inversion H.
  ...
  - econstructor; eauto.
    + eapply Genv.init_mem_transf_partial; eauto.
      intros a b0 TR.
      des a; des b0; monadInv TR.
      unfold transf_function in EQ; monadInv EQ. auto.
...
Theorem transf_program_correct:
  forall sg, 
    forward_simulation
      (LTL.semantics prog needed_stackspace sg )
