...
Proof.
  ...
  constructor. auto. inv me_letvars0. auto. 
Qed.
...
Lemma match_env_empty:
  forall map,
  map.(map_letvars) = nil ->
  match_env map (PTree.empty expr_sym) nil (Regmap.init (Eval Vundef)).
Proof.
...
Lemma match_set_params_init_regs:
  ...
  /\ (forall r, reg_fresh r s2 -> (init_regs tvl rl)#r = Eval Vundef).
Proof.
  induction il; intros.
  ...
  replace (init_regs nil x) with (Regmap.init (Eval Vundef)) in me_vars0, me_letvars0.
  constructor; simpl.
  ...
  constructor; simpl.
  intros id v. repeat rewrite PTree.gsspec. destruct (peq id a); intros.
  subst a. inv H. exists x1; split. auto. rewrite Regmap.gss. auto. 
...
Lemma match_set_locals:
  ...
  (forall r, reg_fresh r s1 -> rs#r = Eval Vundef) ->
...
Proof.
  ...
  - monadInv H2. 
    ...
    + intros id v. simpl. 
      ...
      destr. inv H3. eexists; split; eauto. red; intros; simpl; auto. 
...
Lemma functions_translated:
  forall m (v: expr_sym) (f: CminorSel.fundef),
...
Lemma tr_move_correct:
  forall r1 ns r2 nd cs f sp rs m,
    ...
    exists rs',
      star (fun ge => step ge needed_stackspace ) tge (State cs f sp ns rs m) E0 (State cs f sp nd rs' m) /\
      rs'#r2 = rs#r1 /\
...
Proof.
  ...
  - (exists (rs#r2 <- (rs#r1)); split). 
    apply star_one. eapply exec_Iop. eauto. simpl. auto. 
...
Definition transl_expr_prop 
     (le: letenv) (a: expr) (v: expr_sym) : Prop :=
  forall tm cs f map pr ns nd rd rs dst
    (MWF: map_wf map)
    (TE: tr_expr f.(fn_code) map pr a ns nd rd dst)
    (ME: match_env map e le rs)
    (EXT: mem_lessdef m tm),
  exists rs', exists tm',
     star (fun ge => step ge needed_stackspace ) tge (State cs f sp ns rs tm) E0 (State cs f sp nd rs' tm')
...
Definition transl_exprlist_prop 
     (le: letenv) (al: exprlist) (vl: list expr_sym) : Prop :=
...
Definition transl_condexpr_prop 
  ...
  forall tm cs f map pr ns ntrue nfalse rs 
         ...
         (EXT: mem_lessdef m tm),
  exists rs' tm',
    plus (fun ge => step ge needed_stackspace ) tge (State cs f sp ns rs tm) E0
...
Lemma transl_expr_Evar_correct:
  forall (le : letenv) (id : positive) (v: expr_sym),
  e ! id = Some v ->
  transl_expr_prop le (Evar id) v.
Proof.
 intros; red; intros. inv TE.
  generalize (match_env_find_var _ _ _ _ _ _ _ ME H H1). intro EQ.
...
Lemma transl_expr_Eop_correct:
  forall (le : letenv) (op : operation) (args : exprlist)
         (vargs : list expr_sym) (v : expr_sym),
  eval_exprlist ge (Eval (Vptr sp Int.zero)) e m le args vargs ->
  transl_exprlist_prop le args vargs ->
  eval_operation ge (Eval (Vptr sp Int.zero)) op vargs = Some v ->
  transl_expr_prop le (Eop op args) v.
Proof.
  intros; red; intros. inv TE.  
  (* normal case *) 
  exploit H0; eauto. intros [rs1 [tm1 [EX1 [ME1 [RR1 [RO1 EXT1]]]]]].
  edestruct eval_operation_se as [v' []]; eauto.
...
Lemma transl_expr_Eload_correct:
  forall (le : letenv) (chunk : memory_chunk) (addr : Op.addressing)
         (args : exprlist) (vargs : list expr_sym) (vaddr v : expr_sym),
  eval_exprlist ge (Eval (Vptr sp Int.zero)) e m le args vargs ->
  transl_exprlist_prop le args vargs ->
  Op.eval_addressing ge (Eval (Vptr sp Int.zero)) addr vargs = Some vaddr ->
  Mem.loadv chunk m vaddr = Some v ->
  transl_expr_prop le (Eload chunk addr args) v.
Proof.
  intros; red; intros. inv TE.
  exploit H0; eauto. intros [rs1 [tm1 [EX1 [ME1 [RES1 [OTHER1 EXT1]]]]]].
  edestruct eval_addressing_se as [vaddr' []]; eauto.
  edestruct loadv_mem_rel as [v' []]; eauto.
  exists (rs1#rd <- v'); exists tm1. 
  rewrite Regmap.gss.
  repSplit; auto.
  - (* Exec *)
    eapply star_right. eexact EX1. eapply exec_Iload. eauto. 
    instantiate (1 := vaddr'). rewrite <- H3.
    apply eval_addressing_preserved. exact symbols_preserved.
    rewrite <- H5; unfold Mem.loadv.
    generalize (Mem.lessdef_eqm tm1 _ _ H4).
    intro A; repeat destr; inv A; auto.
    unfold Mem.loadv in H5. rewrite Heqv1 in H5.  congruence.
    traceEq.
  - (* Match-env *)
    eauto with rtlg. 
  - (* Other regs *)
...
Lemma transl_expr_Econdition_correct:
  forall (le : letenv) (a: condexpr) (ifso ifnot : expr)
         (va : bool) v,
    eval_condexpr ge (Eval (Vptr sp Int.zero))  e m le a va ->
    transl_condexpr_prop le a va ->
    eval_expr ge (Eval (Vptr sp Int.zero)) e m le (if va then ifso else ifnot) v ->
...
Lemma transl_expr_Elet_correct:
  forall (le : letenv) (a1 a2 : expr) (v1 v2 : expr_sym),
  eval_expr ge (Eval (Vptr sp Int.zero)) e m le a1 v1 ->
  transl_expr_prop le a1 v1 ->
  eval_expr ge (Eval (Vptr sp Int.zero)) e m (v1 :: le) a2 v2 ->
...
Proof.
  ...
  assert (map_wf (add_letvar map r)) by (eapply add_letvar_wf; eauto). 
  exploit H2. eexact H3. eauto.
  ...
  repSplit; auto.
  - (* Exec *)
    eapply star_trans. eexact EX1. eexact EX2. auto.
  - (* Match-env *)
...
Lemma transl_expr_Eletvar_correct:
  forall (le : list expr_sym) (n : nat) (v : expr_sym),
...
Proof.
  ...
  - destruct H2 as [[A B] | [A B]].
    ...
    + apply match_env_invariant with (rs#rd <- (rs#r)).
      ...
      congruence.
  - rewrite RES1. eapply match_env_find_letvar; eauto. 
  - intros. 
    destruct H2 as [[A B] | [A B]].
...
Lemma transl_expr_Ebuiltin_correct:
  ...
  eval_exprlist ge (Eval (Vptr sp Int.zero)) e m le al vl ->
...
Lemma transl_exprlist_Econs_correct:
  forall (le : letenv) (a1 : expr) (al : exprlist) (v1 : expr_sym)
         (vl : list expr_sym),
  eval_expr ge (Eval (Vptr sp Int.zero)) e m le a1 v1 ->
  transl_expr_prop le a1 v1 ->
  eval_exprlist ge (Eval (Vptr sp Int.zero)) e m le al vl ->
...
Proof.
  ...
  - (* Results *)
    constructor; auto.
    rewrite OTHER2. auto. simpl; auto.
  - (* Other regs *)
...
Lemma transl_condexpr_CEcond_correct:
  forall le cond al vl vb i,
  eval_exprlist ge (Eval (Vptr sp Int.zero)) e m le al vl ->
  transl_exprlist_prop le al vl ->
  eval_condition cond vl = Some vb ->
  Mem.mem_norm m vb = Vint i ->
  transl_condexpr_prop le (CEcond cond al) (negb (Int.eq i Int.zero)).
Proof.
  intros; red; intros. inv TE. 
  exploit H0; eauto. intros [rs1 [tm1 [EX1 [ME1 [RES1 [OTHER1 EXT1]]]]]].
  exists rs1; exists tm1.
  split.
  exploit eval_condition_rel.
  apply wf_mr_ld.
  eapply Val.lessdef_list_inv. eexact RES1. eauto.
  intros [b1 [A B]].
  eapply plus_right. eexact EX1. eapply exec_Icond. eauto.  eauto.
  generalize (rel_norm _ wf_mr_ld wf_mr_norm_ld _ _ _ _ EXT1 B). rewrite H2.
  rewrite lessdef_val. intro C; inv C; auto. auto.
  traceEq.
  split. assumption.
  split. assumption. 
  auto.
...
Lemma transl_condexpr_CEcondition_correct:
  forall le a b c va v,
    eval_condexpr ge (Eval (Vptr sp Int.zero)) e m le a va ->
    transl_condexpr_prop le a va ->
    eval_condexpr ge (Eval (Vptr sp Int.zero)) e m le (if va then b else c) v ->
...
Lemma transl_condexpr_CElet_correct:
  forall le a b v1 v2,
  eval_expr ge (Eval (Vptr sp Int.zero)) e m le a v1 ->
  transl_expr_prop le a v1 ->
  eval_condexpr ge (Eval (Vptr sp Int.zero))  e m (v1 :: le) b v2 ->
...
Proof.
  ...
  repSplit; auto.
  - (* Exec *)
    eapply star_plus_trans. eexact EX1. eexact EX2. traceEq.
  - (* Match-env *)
    eapply match_env_unbind_letvar; eauto.
  - (* Other regs *)
    intros. rewrite OTHER2; auto. 
Qed.

  
Theorem transl_expr_correct:
  forall le a v,
    eval_expr ge (Eval (Vptr sp Int.zero)) e m le a v ->
    transl_expr_prop le a v.
Proof.
  refine
    (eval_expr_ind3 ge (Eval (Vptr sp Int.zero)) e m
                    transl_expr_prop
...
Theorem transl_exprlist_correct:
  forall le a v,
  eval_exprlist ge (Eval (Vptr sp Int.zero)) e m le a v ->
  transl_exprlist_prop le a v.
Proof
  (eval_exprlist_ind3 ge (Eval (Vptr sp Int.zero))  e m
     transl_expr_prop
     transl_exprlist_prop
     transl_condexpr_prop
     transl_expr_Evar_correct
...
Theorem transl_condexpr_correct:
  forall le a v,
    eval_condexpr ge (Eval (Vptr sp Int.zero)) e m le a v ->
    transl_condexpr_prop le a v.
Proof
  (eval_condexpr_ind3 ge (Eval (Vptr sp Int.zero)) e m
     transl_expr_prop
     transl_exprlist_prop
     transl_condexpr_prop
     transl_expr_Evar_correct
...
Theorem transl_exitexpr_correct:
  forall le a x,
  eval_exitexpr ge (Eval (Vptr sp Int.zero)) e m le a x ->
...
- (* XEexit *)
  ...
  auto.
- (* XEjumptable *)
  exploit H4; eauto. intros (nd & A & B).
  exploit transl_expr_correct; eauto. intros (rs1 & tm1 & EXEC1 & ME1 & RES1 & PRES1 & EXT1).
  exists nd, rs1, tm1.
  split. eapply star_right. eexact EXEC1. eapply exec_Ijumptable; eauto.
  generalize (rel_norm _ wf_mr_ld wf_mr_norm_ld _ _ _ _ EXT1 RES1).
  rewrite H0. rewrite lessdef_val. intro C; inv C; auto.
  traceEq.
  auto.
...
- (* XEcondition *)
  ...
  exists nd, rs2, tm2. 
  split. eapply star_trans. apply plus_star. eexact EXEC1. 
  eexact EXEC2. traceEq.
  auto.
- (* XElet *)
  exploit transl_expr_correct; eauto. intros (rs1 & tm1 & EXEC1 & ME1 & RES1 & PRES1 & EXT1).
  assert (map_wf (add_letvar map r))
    by (eapply add_letvar_wf; eauto).
...
with match_stacks: CminorSel.cont -> list RTL.stackframe -> Prop :=
  ...
  | match_stacks_call: forall optid f sp e k r tf n rs cs map nexits ngoto nret rret,
      ...
      match_stacks (Kcall optid f (Eval (Vptr sp Int.zero)) e k) (Stackframe r tf sp n rs :: cs).
...
Inductive match_states: CminorSel.state -> RTL.state -> Prop :=
  | match_state:
      forall f s k sp e m tm cs tf ns rs map ncont nexits ngoto nret rret 
        ...
        (MEXT: mem_lessdef m tm),
      match_states (CminorSel.State f s k (Eval (Vptr sp Int.zero)) e m)
...
Theorem transl_step_correct:
  forall S1 t S2, CminorSel.step ge needed_stackspace  S1 t S2 ->
  forall R1, match_states S1 R1 ->
  exists R2,
    (plus (fun ge => RTL.step ge needed_stackspace ) tge R1 t R2 \/
     (star (fun ge => RTL.step ge needed_stackspace ) tge R1 t R2 /\ lt_state S2 S1))
  /\ match_states S2 R2.
Proof.
  induction 1; intros R1 MSTATE; inversion MSTATE; subst.
  - (* skip seq *)
  ...
  - (* skip return *)   
    ...
    { inv TF. auto. }
    edestruct free_mem_rel as [tm' [A B]]; eauto.
    exploit MemReserve.release_boxes_rel; eauto. intros [tm2' [C D]].
    econstructor; split. 
    + left; apply plus_one. eapply exec_Ireturn; eauto.
      rewrite H4. rewrite A. eauto. 
      inv TF. rewrite H8. eauto.
    + constructor; auto.
  ...
  - (* store *)
    ...
    intros [rs' [tm' [A [B [C [D E]]]]]].
    exploit transl_expr_correct; eauto.
    intros [rs'' [tm'' [I [G [J [K L]]]]]].
    edestruct eval_addressing_se as [vaddr' []]; eauto.
    edestruct storev_rel as [tm''' []]; eauto.
    econstructor; split.
    left; eapply plus_right. eapply star_trans. eexact A. eexact I. reflexivity.
    eapply exec_Istore with (a := vaddr'). eauto.
    replace (rs'' ## rl) with (rs' ## rl). 
    rewrite <- H3. apply eval_addressing_preserved. exact symbols_preserved.
    apply list_map_exten. intros. apply K. auto. eauto.
    traceEq.
    econstructor; eauto. constructor.
  - (* call *)
    inv TS; inv H.
    + (* indirect *)
      ...
      exploit transl_exprlist_correct; eauto.
      intros [rs'' [tm'' [E [F [W [G J ]]]]]].
      exploit functions_translated; eauto. intros [tf' [P Q]].
      econstructor; split.
      left; eapply plus_right. eapply star_trans. eexact A. eexact E. reflexivity.
      eapply exec_Icall; eauto. simpl. rewrite G.
      eapply Genv.find_funct_lessdef; eauto. simpl; auto. 
      apply sig_transl_function; auto.
      traceEq.
      constructor; auto. econstructor; eauto.
    + (* direct *)
      exploit transl_exprlist_correct; eauto.
      intros [rs'' [tm'' [E [F [Z [G Y]]]]]].
      exploit functions_translated; eauto. intros [tf' [P Q]].
      econstructor; split.
      left; eapply plus_right. eexact E.
      eapply exec_Icall; eauto. simpl. rewrite symbols_preserved. rewrite H4. 
      revert P; unfold Genv.find_funct. destr.
      revert P; destr.
      destruct (NormaliseSpec.in_bound_dec (Mem.bounds_of_block m b) 0).
      destruct (Mem.concrete_mem m).
      erewrite Mem.norm_val in Heqv; eauto. inv Heqv.
      eauto.
      rewrite Mem.norm_val in Heqv; inv Heqv.
  ...
  - (* tailcall *)
    ...
    + (* indirect *)
      ...
      intros [rs' [tm' [A [B [C [D X]]]]]].
      exploit transl_exprlist_correct; eauto.
      intros [rs'' [tm'' [E [F [LEOLR [G Y]]]]]].
      exploit functions_translated; eauto. intros [tf' [P Q]].
      exploit match_stacks_call_cont; eauto. intros [U V].
      assert (fn_stacksize tf = fn_stackspace f) by  (inv TF; auto).
      edestruct free_mem_rel as [tm''' [ I J]]; eauto.
      exploit MemReserve.release_boxes_rel; eauto. intros [tm2' [K L]].
      econstructor; split.
      * left; eapply plus_right. eapply star_trans. eexact A. eexact E. reflexivity.
        eapply exec_Itailcall; eauto. simpl. rewrite G.
        eapply Genv.find_funct_lessdef; eauto.
        simpl; auto.
        apply sig_transl_function; auto.
        rewrite H; eauto.
        inv TF. rewrite H9. eauto. 
        traceEq.
      * constructor; auto.
    + (* direct *) 
      exploit transl_exprlist_correct; eauto.
      ...
      * left; eapply plus_right. eexact E.
        ...
        erewrite Mem.norm_val in Heqv; eauto. inv Heqv.
        eauto.
        rewrite Mem.norm_val in Heqv; inv Heqv. eauto.
        apply sig_transl_function; auto.
        rewrite H; eauto.
        inv TF. rewrite H8. eauto. traceEq.
      * constructor; auto.

  - (* builtin *)
    inv TS. 
    exploit transl_exprlist_correct; eauto.
    intros [rs' [tm' [E [F [LEOLR [G K]]]]]].
    edestruct external_call_rel as [tv [tm'' [A [B C]]]]; eauto.
    eapply Val.lessdef_list_inv; eauto.
    econstructor; split.
    + left. eapply plus_right. eexact E.
      eapply exec_Ibuiltin. eauto.
  ...
  - (* internal call *)
    ...
    edestruct alloc_mem_rel as [tm' []]; eauto; try apply Zle_refl.
    destruct (MemReserve.reserve_boxes_rel _ _ _ H8 _ _ H0) as [tm2' [A B]]. 
    econstructor; split.
  ...
  - (* external call *)
    monadInv TF. 
    edestruct external_call_rel as [tvres [tm' [A [B C]]]]; eauto.
    eapply Val.lessdef_list_inv; eauto.
    econstructor; split.
...
Lemma transl_initial_states:
  forall sg S, CminorSel.initial_state prog sg S ->
  exists R, RTL.initial_state tprog sg R /\ match_states S R.
Proof.
  induction 1.
  ...
  - econstructor.
    + eapply (Genv.init_mem_transf_partial _ _ TRANSL); eauto.
      intros a b0 TR.
      unfold CminorSel.fid, fid; des a; des b0; try monadInv TR.
      unfold transl_function in EQ. repeat (revert EQ; destr).
      inv EQ; auto.
...
Proof.
  ...
  generalize (rel_norm _ wf_mr_ld wf_mr_norm_ld _ _ _ _ MEXT LD); rewrite H1.
  rewrite lessdef_val. intro A; inv A; auto.
Qed.

Theorem transf_program_correct:
  forall sg, 
    forward_simulation
      (CminorSel.semantics prog needed_stackspace sg )
